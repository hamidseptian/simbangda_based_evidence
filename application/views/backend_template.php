...
if ($q_kuisioner->num_rows()>0) {
    ...
    where kp.id_user = '$id_user' and kp.id_kuisioner='$id_kuisioner'
    ")->num_rows();
    if ($q_diisi==0) {
...
$timeinsert = date("Y-m-d H:i:s");
   $modules = $this->router->fetch_class();
  // Cek berdasarkan IP, apakah user sudah pernah mengakses hari ini
$s = $this->db->query("SELECT ip FROM visitor WHERE ip='".$ip."' AND date='".$date."'")->num_rows();
$ss = isset($s)?($s):0;
// Kalau belum ada, simpan data user tersebut ke database
if($ss == 0){
    $this->db->query("INSERT INTO visitor(ip, date, hits, online, time, modules) VALUES('".$ip."','".$date."','1','".$waktu."','".$timeinsert."','".$modules."')");
}
// Jika sudah ada, update
else{
    $this->db->query("UPDATE visitor SET hits=hits+1, modules='$modules', last_hits='".$timeinsert."' , online='".$waktu."' WHERE ip='".$ip."' AND date='".$date."'");
}
