 ...
 <tbody>
  ...
  foreach ($lokasi_per_skpd as $k => $v) { 
    ...
    $q_ski = $this->db->query("SELECT lpp.id_paket_pekerjaan as id_paket_pekerjaan,
      ...
      ski.kode_sub_kegiatan, sum(total_anggaran_sub_kegiatan(ski.kode_sub_kegiatan,ski.kode_tahap,ski.id_instansi,ski.kode_kegiatan,ski.kode_program)) AS pagu
      ...
      where lpp.id_instansi = '$id_instansi' and lpp.id_kab_kota='$id_kab_kota'
    ...
    ?>
      ...
      foreach ($q_ski->result() as $key => $d_ski) {
        ...
        $q_hitung_paket = $this->db->query("SELECT distinct  kode_rekening_sub_kegiatan as krsk, id_instansi,
        (SELECT count(id_paket_pekerjaan) from paket_pekerjaan where kode_rekening_sub_kegiatan=krsk and jenis_paket='SWAKELOLA') as totalpaket_swakelola, 
        (SELECT count(id_paket_pekerjaan) from paket_pekerjaan where kode_rekening_sub_kegiatan=krsk and jenis_paket='PENYEDIA' and kategori='KONTRUKSI') as totalpaket_penyedia_konstruksi, 
        (SELECT count(id_paket_pekerjaan) from paket_pekerjaan where kode_rekening_sub_kegiatan=krsk and jenis_paket='PENYEDIA' and kategori='NON KONTRUKSI') as totalpaket_penyedia_non_konstruksi, 
        (SELECT sum(pagu) from paket_pekerjaan where kode_rekening_sub_kegiatan=krsk and id_instansi=id_instansi) as total_pagu_paket 
          from paket_pekerjaan pp where pp.kode_rekening_sub_kegiatan='$kode_rekening_sub_kegiatan' and pp.id_instansi='$id_instansi'")->row_array();
      ...
      <tr>
        ...
        <td><?php echo $d_ski->nama_sub_kegiatan ?></td>
