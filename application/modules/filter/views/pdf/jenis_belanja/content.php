...
<table class="font_laporan border">
 <thead class="header">
    ...
    <th rowspan="3" style="width:430px">Program, Kegiatan, Sub Kegiatan</th>
    <th rowspan="3" style="width:80px">Pagu</th>
    <th rowspan="2" colspan="4">Jenis Belanja (Rp.)</th>
  ...
  <tr>
    <th style="width:80px">Belanja Operasi</th>
    <th style="width:80px">Belanja Modal</th>
    <th style="width:80px">Belanja Tidak Terduga</th>
    <th style="width:80px">Belanja Transfer</th>
 ...
 <!-- <tbody> -->
  ...
  foreach ($program as $key => $value) { 
    ...
    $kegiatan = $this->jenis_belanja->get_kegiatan($id_instansi, $value->kode_rekening_program, $value->kode_bidang_urusan)->result(); ?>
    <tr>
      <td><?php echo $no_program ?></td>
       <td colspan="13" style="font-size:10px"><div><b><?php echo $value->nama_program ?></b></div></td>
     </tr>
    <?php 
    $no_kegiatan=0;
    foreach ($kegiatan as $key => $value_kegiatan) {
      $no_kegiatan++;
      $no_sub_kegiatan = 0;
        $sub_kegiatan = $this->jenis_belanja->get_sub_kegiatan($id_instansi, $value_kegiatan->kode_rekening_kegiatan, $value_kegiatan->kode_rekening_program, $value_kegiatan->kode_bidang_urusan)->result();
    ?>
    <tr>
      <td><?php echo $no_program.'.'.$no_kegiatan ?></td>
       <td colspan="13" style="padding-left:1.5em" ><div class="nama_kegiatan"><b><?php echo $value_kegiatan->nama_kegiatan ?></b></div></td>
     </tr>
     <?php 
     foreach ($sub_kegiatan as $key => $value_sk) {
      ...
      $kategori_sub_kegiatan = $value_sk->kategori;
          ...
          $sumber_dana = $this->jenis_belanja->get_sumber_dana($id_instansi, $value_sk->kode_rekening_sub_kegiatan, $value_sk->kode_rekening_kegiatan, $value_sk->kode_rekening_program, $value_sk->kode_bidang_urusan)->row_array();




          $target = $this->jenis_belanja->get_target($id_instansi, $value_sk->kode_rekening_sub_kegiatan, $bulan)->row_array();
          $realisasi_keuangan = $this->jenis_belanja->get_realisasi_keuangan($id_instansi, $value_sk->kode_rekening_sub_kegiatan, $bulan, $ope)->row_array();
          ...
          $total_paket = $this->jenis_belanja->get_total_paket($id_instansi, $value_sk->kode_rekening_sub_kegiatan)->num_rows();
          $jenis_rutin = $this->jenis_belanja->get_total_paket_perjenis($id_instansi, $value_sk->kode_rekening_sub_kegiatan, "RUTIN")->num_rows();
          $swa = $this->jenis_belanja->get_realisasi_fisik($id_instansi, $value_sk->kode_rekening_sub_kegiatan, $bulan, 'SWAKELOLA', $ope)->row_array();
          $pen = $this->jenis_belanja->get_realisasi_fisik($id_instansi, $value_sk->kode_rekening_sub_kegiatan, $bulan, 'PENYEDIA', $ope)->row_array();
          ...
          $pagu_opd = $this->jenis_belanja->get_pagu_opd($id_instansi)->row_array();
          ...
          $totaldata = $this->jenis_belanja->jumlah_kegiatan($id_instansi);
...
?>
 <!-- </tbody> -->
   ...
   <tr>
    ...
    <tr>
      <td colspan="13" style="font-size:12px"><div><b>Perhitungan Deviasi / Sub Kegiatan</b></div></td>
     
    </tr>
    <tr style="background: #f8b2b2;">
      <td colspan="12">Deviasi Diatas 10%</td>
      <td><center><?= $total_peringatan_dev_fisik_merah  ?></center></td>
      <td><center><?= $total_peringatan_dev_keu_merah  ?></center></td>
    </tr>
    <tr style="background: #fcf3cf;">
      <td colspan="12">Deviasi antara 5% sampai 10%</td>
