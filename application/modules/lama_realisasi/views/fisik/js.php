				...
				if (data.status == true) {
					$.each(data.data, function(k, v) {
						...
						$('#list-sub-kegiatan-pptk-'+ id_user).append('<tr>' +
                								'<td><button id="list-paket" status="collapse" id-pptk="'+ v.id_pptk +'" kode-sub-kegiatan="'+ v.kode_rekening_sub_kegiatan +'" kode-kegiatan="'+ v.kode_kegiatan +'" kode-program="'+ v.kode_program +'" kode-bidang-urusan="'+ v.kode_bidang_urusan +'" class="btn btn-info btn-xs">+</button></td>' +
