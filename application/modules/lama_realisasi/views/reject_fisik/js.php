...
<script>
	...
	function showreject() {
		$('#table-reject-fisik').DataTable({
			processing: true,
			serverSide: true,
			bDestroy: true,
			responsive: true,
			ajax: {
				url: baseUrl('realisasi/dt_reject_fisik/'),
				type: "POST",
				data: {},
			},
			columnDefs: [{
	...
	$('#table-kpa').on('click', '#list-kegiatan', function() {
		...
		tr_detail_kegiatan = '<tr colspan="6" class="card-body formuliri" id="' + id_detail + '">';
		tr_detail_kegiatan += '<td rowspan="1" colspan="6">';
		tr_detail_kegiatan += '<div class="table-responsive">';
				...
				if (data.status == true) {
					$.each(data.data, function(k, v) {
						$('#list-pptk' + id_user_top_parent).append('<tr>' +
							'<th scope="row"><button class="btn btn-info btn-xs" id="list-kegiatan" status="collapse" id-user="' + v.id_user + '">+</button></th>' +
	...
	function listKegiatan(id_user = '') {
		$.ajax({
			url: baseUrl('realisasi/list_kegiatan/'),
			dataType: 'JSON',
			type: 'POST',
			data: {
				id_user: id_user
	...
	function upload_dokumen(id_instansi, id_kpa, id_pptk, id_paket_pekerjaan, kode_rekening_kegiatan, nama_paket, jenis_paket, id_metode) {
	...
	function list_dokumen(id_instansi, id_kpa, id_pptk, id_paket_pekerjaan, kode_rekening_kegiatan, jenis_paket, id_metode) {
		let id = kode_rekening_kegiatan.split('.').join("-");
		$.ajax({
			...
			success: function(data) {
				if (data.status == true) {
					...
					$('#form-upload-dokumen').append('<input type="hidden" id="id_intansi" name="id_instansi" value="' + id_instansi + '">')
						...
						.append('<input type="hidden" id="kode_rekening_kegiatan" name="kode_rekening_kegiatan" value="' + kode_rekening_kegiatan + '">')
	...
	function upload_file(prop, id, dokumen) {
		...
		let form_data_to_append = ['id_instansi', 'id_kpa', 'id_pptk', 'id_paket_pekerjaan', 'jenis_paket', 'kode_rekening_kegiatan'];
			...
			success: function(data) {
				...
				if (data.jumlah_evidence == 0 ) {
					...
					$('#daftar-dokumen-fisik').append('<tr>' +
							...
							'<td style="text-align: center;"> <button class="btn btn-info">' + data.nilai + '</button></td>'
