		...
		if ($tahap==4) {
			...
			$q_user_kpa = $this->db->query("SELECT id_sub_instansi from master_users where id_user='$id_user_kpa'")->row_array();
				...
				trim(substr(usk.kode_rekening_sub_kegiatan,1,15)) = trim(msk.kode_sub_kegiatan)
				 and msk.status=1
	            	where usk.id_user in (SELECT mu.id_user from master_users mu join sub_instansi si on mu.id_sub_instansi=si.id_sub_instansi where si.id_kpa='$id_sub_instansi') and usk.tahun_anggaran='$tahun' and $where_pptk and usk.status='1' and usk.kode_rekening_kegiatan='$kode_kegiatan'");
		}else{
			$q_usk_pptk = $this->db->query("SELECT  usk.kode_rekening_sub_kegiatan, usk.kode_tahap, usk.tahun_anggaran as tahun, msk.nama_sub_kegiatan,
				...
				trim(substr(usk.kode_rekening_sub_kegiatan,1,15)) = trim(msk.kode_sub_kegiatan)
				 and msk.status=1
	            	where usk.id_user='$id_user_pptk' and usk.tahun_anggaran='$tahun' and $where_pptk and usk.status='1' and usk.kode_rekening_kegiatan='$kode_kegiatan'");
		}
