...
<table class="font_laporan border">
 ...
 <!-- <tbody> -->
  ...
  foreach ($paket->result_array() as $k_paket => $v_paket) { 
    ...
    $ski = $this->db->query("SELECT  nama_sub_kegiatan, kategori, jenis_sub_kegiatan, keterangan, pagu from v_sub_kegiatan_apbd where kode_rekening_sub_kegiatan='$kode_sub_kegiatan' and tahun='$tahun' and kode_tahap='$tahap' and id_instansi='$id_instansi'")->row_array();
    ...
    $nama_sub_kegiatan = $ski['kategori'] == 'Unit Pelaksana' ? $ski['nama_sub_kegiatan'] : $ski['nama_sub_kegiatan'] ;
          ...
          if ($v_paket['id_kontrak_pekerjaan']==null && $v_paket['jenis_paket']=='PENYEDIA') {
            ...
            $realisasi_keuangan = $this->realisasi_akumulasi_model->get_realisasi_keuangan($v_paket['id_instansi'], $kode_sub_kegiatan, bulan_aktif(), '<=', $v_paket['tahun'], $v_paket['kode_tahap'])->row_array();
            $persen_realisasi_keuangan  = round(($realisasi_keuangan['total_realisasi'] / $ski['pagu']) * 100, 2);
      ...
      <td><?php echo $nama_sub_kegiatan ?></td>
       ...
       <td align="center">
        <?php 
          if ($v_paket['jenis_paket']=='RUTIN') {
             $nilai_paket = $persen_realisasi_keuangan;//$nilai_rutin;
          }else{
