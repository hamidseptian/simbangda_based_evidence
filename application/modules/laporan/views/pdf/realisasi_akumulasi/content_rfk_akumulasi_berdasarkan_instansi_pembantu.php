...
$total_sisa_anggaran_semua = 0;
  ...
  foreach ($program as $key => $value) { 
    ...
    $kegiatan = $this->realisasi_akumulasi_model->get_kegiatan_berdasarkan_instansi_pembantu($id_instansi, $value->kode_program, $tahun, $kode_tahap, $id_instansi_pembantu)->result(); ?>
    ...
    foreach ($kegiatan as $key => $value_kegiatan) {
      ...
      $no_kegiatan++;
      $no_sub_kegiatan = 0;
        $sub_kegiatan = $this->realisasi_akumulasi_model->get_sub_kegiatan_berdasarkan_instansi_pembantu($id_instansi, $value_kegiatan->kode_kegiatan, $tahun, $kode_tahap, $id_instansi_pembantu);
    ?>
    ...
    $sisa_anggaran_kegiatan =0;
     ...
     foreach ($sub_kegiatan->result() as $key => $value_sk) {
      ...
      $krsk = $value_sk->kode_sub_kegiatan;
      $total_pagu_sub_kegiatan_instansi +=$value_sk->pagu ;
          ...
          // $sumber_dana = $this->realisasi_akumulasi_model->get_sumber_dana($id_instansi, $value_sk->kode_sub_kegiatan, $value_sk->kode_rekening_kegiatan, $value_sk->kode_rekening_program, $value_sk->kode_bidang_urusan)->row_array();

          $q_pptk = $this->db->query("SELECT mu.full_name as pptk, mu2.full_name  as kpa from users_sub_kegiatan usk left join master_users mu on usk.id_user = mu.id_user
            left join sub_instansi si on mu.id_sub_instansi = si.id_sub_instansi 
            left join master_users  mu2 on si.id_kpa = mu2.id_sub_instansi

              where usk.id_instansi = '$id_instansi' and usk.tahun_anggaran='$tahun' and usk.kode_tahap = '$tahap' and usk.status='1' and kode_rekening_sub_kegiatan ='$krsk'
            ");
          $kpa = $q_pptk->row_array()['kpa'];
          $kumpul_pptk = [];
          if ($q_pptk->num_rows()>1) {
            $urutan_pptk = 0;
            foreach ($q_pptk->result_array() as $k_pptk => $v_pptk) {
            $urutan_pptk++;
              $pptk = $urutan_pptk.'. '.$v_pptk['pptk'];
              array_push($kumpul_pptk, $pptk);
            }
            $pptk = join("<br>",$kumpul_pptk);
          }else{
            $pptk = $q_pptk->row_array()['pptk'];

          }
          


          $target = $this->realisasi_akumulasi_model->get_target($id_instansi, $value_sk->kode_sub_kegiatan, $bulan, $tahap, $tahun)->row_array();
          $realisasi_keuangan = $this->realisasi_akumulasi_model->get_realisasi_keuangan($id_instansi, $value_sk->kode_sub_kegiatan, $bulan, $ope, $tahun, $tahap)->row_array();
          ...
          if ($ope=='=') {
            ...
            $nilai_persen_target_keuangan = $value_sk->pagu == 0 ? 0 : ($target['target_keuangan'] / $value_sk->pagu) * 100 ; 
          ...
          $swa = $this->realisasi_akumulasi_model->get_realisasi_fisik($id_instansi, $value_sk->kode_sub_kegiatan, $bulan, 'SWAKELOLA', $ope, $tahun, $tahap)->row_array();
          $pen = $this->realisasi_akumulasi_model->get_realisasi_fisik($id_instansi, $value_sk->kode_sub_kegiatan, $bulan, 'PENYEDIA', $ope, $tahun, $tahap)->row_array();
 ...
 if ($total_paket != 0) {
      ...
      ?>
       ...
       <?php 
          ...
          $total_tk_semua+=$target_keuangan;
          $total_rk_semua+=$realisasi_keuangan['total_realisasi'];
          $total_sisa_anggaran_semua+=$sisa_pagu;
  ...
  <?php 
        ...
        $tampilkan_persen_tk_kegiatan = $pagu_kegiatan == 0 ? 0 : ($nilai_tk_kegiatan / $pagu_kegiatan) * 100;
        $tampilkan_persen_rk_kegiatan = $pagu_kegiatan == 0 ? 0 : ($nilai_rk_kegiatan / $pagu_kegiatan) * 100;
      ...
      $tampilkan_persen_tk_program = $pagu_program == 0 ? 0 : ($nilai_tk_program / $pagu_program) * 100;
      $tampilkan_persen_rk_program = $pagu_program == 0 ? 0 : ($nilai_rk_program / $pagu_program) * 100;
...
<table class="font_laporan border table">
 ...
 <tbody>
  ...
  <tr style="background: #e8daef">
    ...
    <th align="left" colspan="16"> <?php echo $v_program['nama_program'] ?></th>
    ...
    foreach ($v_program['data_kegiatan'] as $k_kegiatan => $v_kegiatan) { 
      ...
      ?>
       ...
       <tr style="background: #d6eaf8 ">
        ...
        <th align="left" colspan="16"> <?php echo $v_kegiatan['nama_kegiatan'] ?></th>
    
      </tr>
<?php  
      foreach ($v_kegiatan['data_sub_kegiatan'] as $k_ski => $v_sub_kegiatan) { 

          $bobot_ski =($v_sub_kegiatan['pagu_sub_kegiatan']/$total_pagu_sub_kegiatan_instansi)*100;
          $tft_ski =$v_sub_kegiatan['persen_tf_sub_kegiatan'] * $bobot_ski /100;
          $rft_ski =$v_sub_kegiatan['persen_rf_sub_kegiatan'] * $bobot_ski /100;
        ...
        ?>
          ...
          <tr>
            ...
            <td align="center"><?php echo round($bobot_ski,2) ;//round($v_sub_kegiatan['bobot_ski'],2) ?></td>
            <td align="center" <?php echo $bg_tf ?>> <?php echo round($v_sub_kegiatan['persen_tf_sub_kegiatan'],2) ?></td>
            <td align="center"><?php echo round($tft_ski,2) ;//round($v_sub_kegiatan['tft_ski'],2) ?></td>
            <td align="center"  <?php echo $bg_rf ?>> <?php echo round($v_sub_kegiatan['persen_rf_sub_kegiatan'],2) ?></td>
            <td align="center"><?php echo round($rft_ski,2) ;//round($v_sub_kegiatan['rft_ski'],2) ?></td>
...
<?php 
            ...
            $persen_tk_total = $total_tk_semua / $total_pagu_sub_kegiatan_instansi * 100 ; 
            $persen_rk_total = $total_rk_semua / $total_pagu_sub_kegiatan_instansi * 100 ; 



            $dev_fisik_total = $total_rft_ski - $total_tft_ski ; 
            $dev_keu_total = $persen_rk_total - $persen_tk_total ; 
            ...
            elseif ($dev_keu_total <-5  && $dev_keu_total >=-10) {
              $warna_peringatan_dev_keu_total = 'background: #fcf3cf';
            }
            elseif ($dev_keu_total <=0  && $dev_keu_total >=-5) {
...
<tfoot>
  <tr >
    <th colspan="4">Total / Pencapaian</th>
    <th align="right"> <?php echo number_format($total_pagu_sub_kegiatan_instansi) ?> </th>
    <th> - </th>
    <th> - </th>
    <th> <?php echo round($total_bobot_ski,2) ?> </th>
    <th colspan="2"> <?php echo round($total_tft_ski,2) ?> </th>
    <th colspan="2"> <?php echo round($total_rft_ski,2) ?> </th>
    <th style="<?php echo $warna_peringatan_dev_fisik_total ?>"> <?php echo round($dev_fisik_total,2) ?> </th>
    <th align="right"> <?php echo number_format($total_tk_semua) ?> </th>
    <th> <?php echo round($persen_tk_total,2) ?> </th>
    <th align="right"> <?php echo number_format($total_rk_semua) ?> </th>
    <th> <?php echo round($persen_rk_total,2) ?> </th>
    <th> <?php echo round($dev_keu_total,2) ?> </th>
    <th align="right"> <?php echo number_format($total_sisa_anggaran_semua) ?> </th>
  </tr>
</tfoot>
</table>
