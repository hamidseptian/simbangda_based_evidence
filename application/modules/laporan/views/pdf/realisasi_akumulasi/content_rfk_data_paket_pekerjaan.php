  ...
  foreach ($program as $key => $value) { 
    ...
    $sisa_anggaran_kegiatan =0;
     ...
     foreach ($sub_kegiatan->result() as $key => $value_sk) {
      ...
      if ($kode_tahap==4) {
        if ($kode_tahap==$tahap) {
          $data_paket = $this->db->query("SELECT * from v_paket where id_instansi='$id_instansi' and tahun='$tahun' and kode_rekening_sub_kegiatan='$krsk' and status='1'");
        }else{
        ...
        $data_paket = $this->db->query("SELECT * from v_paket where id_instansi='$id_instansi' and tahun='$tahun' and kode_rekening_sub_kegiatan='$krsk' and kode_tahap='$tahap'");
      }
      $no_paket = 0;
      $kumpul_paket=[];
        foreach ($data_paket->result() as $k_paket => $v_paket) {
          ...
          if ($tahun<2023) {
            $nilai_paket = $v_paket->jenis_paket=='RUTIN' ?  $realisasi_rutin_ok : ($nilai_evidence['rf'] =='' ? 0 : $nilai_evidence['rf']);
          }
          elseif ($tahun==2023) {
            $nilai_paket = $v_paket->jenis_paket=='RUTIN' ?  round($persen_realisasi_keuangan,2) : ($nilai_evidence['rf'] =='' ? 0 : $nilai_evidence['rf']);
          }else{
            $nilai_paket = ($nilai_evidence['rf'] =='' ? 0 : $nilai_evidence['rf']);
...
<table class="font_laporan border table">
 <thead class="header">
  ...
  <tr>
     <th  style="width:85px">Tahapan APBD <br>  <span style="color:blue">Paket Ditambahkan Pada</span></th>
...
<?php foreach ($v_sub_kegiatan['data_paket'] as $k_paket => $v_paket) { ?>
       <tr>
            ...
            <td style="color:blue"> <?php echo pilihan_nama_tahapan($v_paket['kode_tahap']) ?></td>
            <td> </td>
...
<tfoot>
   ...
   <tr>
     ...
     <td align="center" colspan="4"> 
      <?php echo $total_paket_keseluruhan ?> Paket <br>  
      <?php echo $total_paket_rutin ?> Rutin <br>  
      <?php echo $total_paket_swakelola ?> Swakelola <br>  
      <?php echo $total_paket_penyedia ?> Penyedia <br>  
    </td>
