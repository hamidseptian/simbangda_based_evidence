  ...
  foreach ($program as $key => $value) { 
    ...
    // $pagu_kegiatan =0;
          ...
          // $target_keuangan_bulanan = $target['target_keuangan_bulanan'];
  			...
  			$jenis_swakelola = $this->realisasi_akumulasi_model->get_total_paket_perjenis($id_instansi, $value_sk->kode_rekening_sub_kegiatan, "SWAKELOLA", $tahun, $tahap)->num_rows();
  			$jenis_penyedia = $this->realisasi_akumulasi_model->get_total_paket_perjenis($id_instansi, $value_sk->kode_rekening_sub_kegiatan, "PENYEDIA", $tahun, $tahap)->num_rows();
          ...
          $dev_fisik = round($total_realisasi_fisik - $target_fisik,2);
          $dev_keu = round($persen_realisasi_keuangan - $persen_target_keuangan,2);
      ...
      $persen_capaian_fisik_kegiatan  = $tampilkan_persen_tf_kegiatan == 0 ? 0 : ($tampilkan_persen_rf_kegiatan / $tampilkan_persen_tf_kegiatan) * 100 ; 
      $persen_capaian_keuangan_kegiatan  = $tampilkan_persen_tk_kegiatan == 0 ? 0 : ($tampilkan_persen_rk_kegiatan / $tampilkan_persen_tk_kegiatan) * 100 ; 
    ...
    $persen_capaian_fisik_program  = $tampilkan_persen_tf_program == 0 ? 0 : ($tampilkan_persen_rf_program / $tampilkan_persen_tf_program) * 100 ; 
    $persen_capaian_keuangan_program  = $tampilkan_persen_tk_program == 0 ? 0 : ($tampilkan_persen_rk_program / $tampilkan_persen_tk_program) * 100 ; 
...
>Program, Kegiatan, Sub Kegiatan</th><!-- <th
rowspan="3" style="width:80px">Pagu Anggaran</th> --> <th colspan="4">Fisik
...
<tfoot>
	...
	<tr>
		...
		elseif ($deviasi_fisik_total <=0  && $deviasi_fisik_total >=-5) {
			$warna_peringatan_deviasi_fisik_total = 'background: #d5f5e3';
		}else{
			$warna_peringatan_deviasi_fisik_total = 'background: #ff7cfd';
		}
		...
		elseif ($deviasi_keuangan_total <=0  && $deviasi_keuangan_total >=-5) {
			$warna_peringatan_deviasi_keuangan_total = 'background: #d5f5e3';
		}else{
			$warna_peringatan_deviasi_keuangan_total = 'background: #ff7cfd';
		}
		...
		<th><?php echo round($tf_pencapaian,2) ?></th>
		<th><?php echo round($rf_pencapaian,2) ?></th>
