...
<table class="font_laporan border table">
 <thead class="header">
  ...
  foreach ($instansi_yang_menyampaikan as $k => $v) { 
    ...
    $no++;
    $warna_instansi ="style='background: #d7e3fb '";
    $warna_permasalahan ="";

    ?>
    <tr <?php echo $warna_instansi ?>>
      <th align="left"><?php echo $no ?></th>
      <th colspan="3" align="left"><?php echo $v->nama_instansi ?></th>
    </tr>
  <?php
   $q_sk_permasalahan = $this->db->query("SELECT psk.kode_sub_kegiatan, msk.nama_sub_kegiatan,ski.kategori, ski.jenis_sub_kegiatan , ski.keterangan  from permasalahan_sub_kegiatan psk 
    left join master_sub_kegiatan msk on(trim(substr(psk.kode_sub_kegiatan,1,15)) = trim(msk.kode_sub_kegiatan))
    left join sub_kegiatan_instansi ski on psk.id_instansi=ski.id_instansi and psk.kode_sub_kegiatan=ski.kode_sub_kegiatan and psk.tahun=ski.tahun
     where psk.id_instansi='$idinstansi' and psk.tahun='$tahun' and psk.permasalahan!='Tidak ada permasalahan' group by psk.kode_sub_kegiatan")->result();
   $no_sk = 0;
   foreach ($q_sk_permasalahan  as $k_sk_permasalahan => $v_sk_permasalahan) { 
    ...
    ?>
     <tr <?php echo $warna_permasalahan ?>>
        <td valign="top" align="left"><?php echo $no.'.'.$no_sk ?></td>
        <td valign="top" align="left"><?php echo $nama_sub_kegiatan ?></td>
        <td valign="top" align="left">
            <ol>
          <?php 
          $q_show_masalah = $this->db->query("SELECT permasalahan from permasalahan_sub_kegiatan where id_instansi='$idinstansi' and kode_tahap='$kode_tahap' and kode_sub_kegiatan='$ksk'  and tahun='$tahun' and permasalahan!='Tidak ada permasalahan'")->result();
