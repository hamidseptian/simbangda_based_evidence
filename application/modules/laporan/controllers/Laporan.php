...
class Laporan extends MY_Controller { 
	...
	public function berita_acara()
	{
		...
		$data['opd']					= $realisasi_akumulasi->get_opd();
		$data['config']					= $this->db->get('config')->result_array();
		$page               	= 'laporan/berita_acara/index';
		$data['link']       	= $this->router->fetch_method();
		$data['menu']       	= $this->load->view('layout/menu', $data, true);
		$data['extra_css']  	= $this->load->view('laporan/berita_acara/css', $data, true);
		$data['extra_js']   	= $this->load->view('laporan/berita_acara/js', $data, true);
		$data['modal']      	= $this->load->view('laporan/berita_acara/modal', $data, true);
		$this->template->load('backend_template', $page, $data);
	}
  ...
  //       $save = $list_directory.'/'.$filenya;
	...
	public function pencapaian_semua_skpd()
	{
		$breadcrumbs    = $this->breadcrumbs;
		$rekap_asisten  = $this->rekap_asisten_model;
		...
		$breadcrumbs->add('Home', base_url());
		$breadcrumbs->add('Laporan', base_url($this->router->fetch_class()));
		$breadcrumbs->add('Pencapaian Semua OPD', base_url());
		$breadcrumbs->render();

		$data['title']      	= "Pencapaian Semua OPD";
		$data['icon']       	= "metismenu-icon fas fa-file-signature";
		$data['description']	= "Laporan realisasi fisik dan keuangan per Asisten";
		$data['breadcrumbs']	= $breadcrumbs->render();
		$page               	= 'laporan/pencapaian_semua_skpd/index';

		$data['config']					= $this->db->get('config')->result_array();
		$data['link']       	= $this->router->fetch_method();
		$data['menu']       	= $this->load->view('layout/menu', $data, true);
		$data['extra_css']  	= $this->load->view('laporan/pencapaian_semua_skpd/css', $data, true);
		$data['extra_js']   	= $this->load->view('laporan/pencapaian_semua_skpd/js', $data, true);
		$data['modal']      	= $this->load->view('laporan/pencapaian_semua_skpd/modal', $data, true);
		$this->template->load('backend_template', $page, $data);
	}

	public function rekap_semua_opd_detail()
	{
		$breadcrumbs    = $this->breadcrumbs;
		$rekap_asisten  = $this->rekap_asisten_model;
		...
		$data['description']	= "Laporan realisasi fisik dan keuangan per Asisten";
		$data['breadcrumbs']	= $breadcrumbs->render();
		$page               	= 'laporan/views/rekap_semua_opd_detail/index';

		$data['config']					= $this->db->get('config')->result_array();
		$data['link']       	= $this->router->fetch_method();
		$data['menu']       	= $this->load->view('layout/menu', $data, true);
		$data['extra_css']  	= $this->load->view('laporan/views/rekap_semua_opd_detail/css', $data, true);
		$data['extra_js']   	= $this->load->view('laporan/views/rekap_semua_opd_detail/js', $data, true);
		$data['modal']      	= $this->load->view('laporan/views/rekap_semua_opd_detail/modal', $data, true);
		$this->template->load('backend_template', $page, $data);
	}
 ...
 //    }
    ...
    // $id_instansi = 12;
		...
		$grafik = $this->db->query("SELECT * from grafik where id_instansi='$id_instansi' and tahun='$tahun' and kode_tahap = '$tahap' and bulan='$bulan'")->row_array();
        ...
        if ($kategori_penampilan_laporan=='rfk_akumulasi') {
			...
			if($tahun <=2022){
				...
				$html =  $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi_rutin_sd_rk', $data, true);
			}
        ...
        elseif ($kategori_penampilan_laporan=='rfk_akumulasi_struktur_instansi') {

				$id_user_kpa 				= $this->input->get('id_user_kpa');
				$id_user_pptk 				= $this->input->get('id_user_pptk');
				$program = $this->realisasi_akumulasi_model->get_program_berdasarkan_struktur_instansi($id_instansi, $tahap, $tahun, $id_user_kpa, $id_user_pptk);
				$data['program']=$program;
				$data['id_user_kpa']=$id_user_kpa;
				$data['id_user_pptk']=$id_user_pptk;


				$judul_penampilan_laporan = "Laporan Realisasi Fisik Dan Keuangan<br>".$judul_laporan;
			   if($tahun <=2022){
					$html = "Fitur ini tidak tersedia pada data tahun 2022";// $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi', $data, true);
				}else{
					$html =  $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi_berdasarkan_struktur_instansi', $data, true);
				}
        	$mpdf->SetMargins(0, 0, 42);
        }
        elseif ($kategori_penampilan_laporan=='rfk_akumulasi_instansi_pembantu') {

				$id_instansi_pembantu 				= $this->input->get('id_instansi_pembantu');
				$q_instansi_pembantu = $this->db->query("SELECT nama_instansi_teknis, jenis_teknis from instansi_pembantu_teknis where id_instansi_pembantu_teknis='$id_instansi_pembantu'")->row_array();
				$program = $this->realisasi_akumulasi_model->get_program_berdasarkan_instansi_pembantu($id_instansi, $tahap, $tahun, $id_instansi_pembantu);
				$data['program']=$program;
				$data['id_user_kpa']=$id_user_kpa;
				$data['id_instansi_pembantu']=$id_instansi_pembantu;

				if ($id_instansi_pembantu=='non') {
					$judul_penampilan_laporan = "Laporan realisasi fisik dan keuangan<br>SKPD [Non Instansi Pembantu Teknis]"."<br>".$judul_laporan;
					# code...
				}else{
					$judul_penampilan_laporan = "Laporan realisasi fisik dan keuangan berdasarkan instansi pembantu<br>".$q_instansi_pembantu['jenis_teknis']. ' - '.$q_instansi_pembantu['nama_instansi_teknis']."<br>".$judul_laporan;

				}
			   if($tahun <=2022){
					$html = "Fitur ini tidak tersedia pada data tahun 2022";// $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi', $data, true);
				}else{
					$html =  $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi_berdasarkan_instansi_pembantu', $data, true);
				}
        	$mpdf->SetMargins(0, 0, 44);
        }
      ...
      //   	$mpdf->SetMargins(0, 0, 42);
      //   }
        elseif ($kategori_penampilan_laporan=='kontrak_pekerjaan') {
        	$judul_penampilan_laporan = "Daftar Kontrak Pekerjaan<br>".nama_instansi($id_instansi);
        ...
        elseif ($kategori_penampilan_laporan=='rfk_berdasarkan_data_sumber_dana') {
		    ...
		    $daftar_skpd = $this->realisasi_akumulasi_model->get_skpd_berdasarkan_sumber_dana($tahap, $tahun, $jenis_sumber_dana)->result();

		    $data['daftar_skpd']=$daftar_skpd;
		    $jenis_sumber_dana_terkolom = ['pad','dau','dak','dbh','lainnya'];
			if (in_array($jenis_sumber_dana, $jenis_sumber_dana_terkolom)) {
			    $data['jenis_sumber_dana']=strtoupper($jenis_sumber_dana)	;
			    $judul_laporan_jenis_sumber_dana = strtoupper($jenis_sumber_dana) ; 
			}else{
			    $data['jenis_sumber_dana']= $nama_jenis_sumber_dana;
			    $judul_laporan_jenis_sumber_dana = $nama_jenis_sumber_dana;

			}
		    $data['field_jenis_sumber_dana']=$jenis_sumber_dana;
        	if ($id_instansi=='semua_opd') {
        		
        	$judul_penampilan_laporan = "Laporan realisasi fisik dan keuangan berdasarkan Sumber Dana ".$judul_laporan_jenis_sumber_dana." Per Sub Kegiatan<br>".$judul_laporan;
		    // $html =  $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi_berdasarkan_sumber_dana', $data, true);
		    $html =  $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi_berdasarkan_sumber_dana_gabungan_semua_opd_dengan_bobot', $data, true);
        	}else{
		    $program = $this->realisasi_akumulasi_model->get_program_berdasarkan_sumber_dana($id_instansi, $tahap, $tahun, $jenis_sumber_dana);
		    $data['program']=$program->result();
        	$judul_penampilan_laporan = "Laporan realisasi fisik dan keuangan berdasarkan Sumber Dana ".$judul_laporan_jenis_sumber_dana." Per Sub Kegiatan<br>".$judul_laporan;
		    // $html =  $this->load->view('laporan/pdf/realisasi_akumulasi/content_rfk_akumulasi_berdasarkan_sumber_dana', $data, true);
...
ini_set("pcre.backtrack_limit", "5000000");
 ...
 $data['identitas']=$identitas;
	...
	public function pdf_laporan_rekap_asisten()
	{
		...
		$pagu_keseluruhan =0;
		foreach ($asisten_1 as $satu) {
			$pagu_asisten_1 +=$satu->pagu_total;
		}
		foreach ($asisten_2 as $dua) {
			$pagu_asisten_2 +=$dua->pagu_total;
		}
		foreach ($asisten_3 as $tiga) {
			$pagu_asisten_3 +=$tiga->pagu_total;
		}



		   $query_sql_inspektorat = 'g.target_fisik_akumulasi as target_fisik,
    ...
    $query_inspektorat  = $this->db->query("SELECT
                               		mi.nama_instansi, 
                                   $query_sql_inspektorat,
                                    g.last_update
	    ...
	    $data['pagu_asisten_1']=$pagu_asisten_1;
	    $data['pagu_asisten_2']=$pagu_asisten_2;
	    $data['pagu_asisten_3']=$pagu_asisten_3;
	    ...
	    $data['inspektorat']=$query_inspektorat->row_array();
  ...
  //                                 ");
	...
	public function pdf_laporan_pencapaian_semua_skpd()
	{
		$mpdf = new \Mpdf\Mpdf([
		    'mode' => 'utf-8',
		    'format' => 'legal',
		...
		$data_diambil 				= $this->input->get('data');
		$tahap 				= $this->input->get('tahap');
		$tahun 				= $this->input->get('tahun');
		$kategori 				= $this->input->get('kategori');
		$asisten 				= $this->input->get('asisten');

	    if ($data_diambil=='Semester 1') {
	    	$bulan_mulai = 1;
	    	$bulan_selesai  =6;
	    }else{
	    	$bulan_mulai = 7;
		...
		$identitas = $this->db->get('identitas')->row_array();
		if ($asisten==1) {
			$where_asisten = "AND id_parent = '204'";
			$where_pagu_bobot = "SELECT id_instansi from master_instansi where kategori='OPD' and is_active='1' $where_asisten";
			$q_where_pagu_bobot = "AND id_instansi in ($where_pagu_bobot)";
		}
		elseif ($asisten==2) {
			$where_asisten = "AND id_parent = '205'";
			$where_pagu_bobot = "SELECT id_instansi from master_instansi where kategori='OPD' and is_active='1' $where_asisten";
			$q_where_pagu_bobot = "AND id_instansi in ($where_pagu_bobot)";
			# code...
		}
		elseif ($asisten==3) {
			$where_asisten = "AND id_parent = '206'";
			$where_pagu_bobot = "SELECT id_instansi from master_instansi where kategori='OPD' and is_active='1' $where_asisten";
			$q_where_pagu_bobot = "AND id_instansi in ($where_pagu_bobot)";
			# code...
		}
		elseif ($asisten=='inspektorat') {
			$where_asisten = "AND id_parent is null";
			$where_pagu_bobot = "SELECT id_instansi from master_instansi where kategori='OPD' and is_active='1' $where_asisten";
			$q_where_pagu_bobot = "AND id_instansi in ($where_pagu_bobot)";
			# code...
		}
			...
			$where_asisten = "";
			$where_pagu_bobot = "SELECT id_instansi from master_instansi where kategori='OPD' and is_active='1'";
			$q_where_pagu_bobot = "AND id_instansi in ($where_pagu_bobot)";
			# code...
		}


		$opd = $this->db->query("SELECT id_instansi, nama_instansi from master_instansi where kategori='OPD' and is_active='1' $where_asisten")->result_array();


		$judul_file = "Laporan Rekap Realisasi Fisik Dan Keuangan Per SKPD <br>".$data_diambil.' Tahun '.$tahun;
	    $data['judul_laporan']= $judul_file;
	    $data['q_where_pagu_bobot']=$q_where_pagu_bobot;
	    ...
	    $html =  $this->load->view('laporan/pdf/pencapaian_semua_skpd/content', $data, true);

	    $header =  $this->load->view('laporan/pdf/pencapaian_semua_skpd/header', $data, true);
	    $footer =  $this->load->view('laporan/pdf/pencapaian_semua_skpd/footer', $data, true);
  ...
  //                                 ");
	...
	public function pdf_laporan_ratarata_realisasi()
	{
	    ...
	    $skpd_terurut = [];
            ...
            $hitung_ketcap_f_ST = 0;
            $hitung_ketcap_f_T = 0;
            $hitung_ketcap_f_S = 0;
            $hitung_ketcap_f_R = 0;
            $hitung_ketcap_f_SR = 0;


            $hitung_ketcap_k_ST = 0;
            $hitung_ketcap_k_T = 0;
            $hitung_ketcap_k_S = 0;
            $hitung_ketcap_k_R = 0;
            $hitung_ketcap_k_SR = 0;


            $hitung_dev_f_merah = 0;
            $hitung_dev_f_kuning = 0;
            $hitung_dev_f_hijau = 0;
            $hitung_dev_f_ungu = 0;

            $hitung_dev_k_merah = 0;
            $hitung_dev_k_kuning = 0;
            $hitung_dev_k_hijau = 0;
            $hitung_dev_k_ungu = 0;
            ...
            $capaian_keuangan = $v->realisasi_keuangan == '' ? 0 : ( $v->target_keuangan == 0 ? INF : ($v->realisasi_keuangan / $v->target_keuangan * 100) );
             ...
             if ($v->target_fisik<$v->target_keuangan) {
              ...
              $blok_tf = '';
	    		...
	    		else if ($capaian_fisik<=90 && $capaian_fisik>80) {
	    			$hitung_ketcap_f_T++;
	    			$ketcap_f = 'T';
	    			$warcap_f = 'background:#DCFCE7';
	    		}
	    		else if ($capaian_fisik<=80 && $capaian_fisik>60) {
	    			$hitung_ketcap_f_S++;
	    			$ketcap_f = 'S';
	    			$warcap_f = 'background:#FEF9C2';
	    		}
	    		else if ($capaian_fisik<=60 && $capaian_fisik>50) {
	    			$hitung_ketcap_f_R++;
	    			$ketcap_f = 'R';
				...
				if ($capaian_keuangan>90) {
					$hitung_ketcap_k_ST++;
	    			$ketcap_k = 'ST';
	    			$warcap_k = 'background: #7BF1A8';
	    		}
	    		else if ($capaian_keuangan<=90 && $capaian_keuangan>80) {
	    			$hitung_ketcap_k_T++;
	    			$ketcap_k = 'T';
	    			$warcap_k = 'background:#DCFCE7';
	    		}
	    		else if ($capaian_keuangan<=80 && $capaian_keuangan>60) {
	    			$hitung_ketcap_k_S++;
	    			$ketcap_k = 'S';
	    			$warcap_k = 'background:#FEF9C2';
	    		}
	    		else if ($capaian_keuangan<=60 && $capaian_keuangan>50) {
	    			$hitung_ketcap_k_R++;
	    			$ketcap_k = 'R';
	    ...
	    $statistik = [
			'hitung_ketcap_f_ST'=>$hitung_ketcap_f_ST,
			'hitung_ketcap_f_T'=>$hitung_ketcap_f_T,
			'hitung_ketcap_f_S'=>$hitung_ketcap_f_S,
			'hitung_ketcap_f_R'=>$hitung_ketcap_f_R,
			'hitung_ketcap_f_SR'=>$hitung_ketcap_f_SR,
			'hitung_ketcap_k_ST'=>$hitung_ketcap_k_ST,
			'hitung_ketcap_k_T'=>$hitung_ketcap_k_T,
			'hitung_ketcap_k_S'=>$hitung_ketcap_k_S,
			'hitung_ketcap_k_R'=>$hitung_ketcap_k_R,
			'hitung_ketcap_k_SR'=>$hitung_ketcap_k_SR,



			'hitung_dev_f_merah'=>$hitung_dev_f_merah,
			'hitung_dev_f_kuning'=>$hitung_dev_f_kuning,
			'hitung_dev_f_hijau'=>$hitung_dev_f_hijau,
			'hitung_dev_f_ungu'=>$hitung_dev_f_ungu,
			'hitung_dev_k_merah'=>$hitung_dev_k_merah,
			'hitung_dev_k_kuning'=>$hitung_dev_k_kuning,
			'hitung_dev_k_hijau'=>$hitung_dev_k_hijau,
			'hitung_dev_k_ungu'=>$hitung_dev_k_ungu,
	    ];
    ...
    public function list_kecamatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $id_kota    = sbe_crypt($this->input->post('id_kota'), 'D'); 
                ...
                $kecamatan = $this->db->query("SELECT id_kecamatan, nama_kecamatan FROM kecamatan where id_kota = '$id_kota'");

            if ($kecamatan->num_rows() > 0) {
                $output['status']     = true;
                $output['message']     = 'Sukses';
                foreach ($kecamatan->result() as $key => $value) {
                    $output['data'][$key]['id']         =  sbe_crypt($value->id_kecamatan, 'E') ;
   ...
   $dompdf->stream('laporan.pdf',array("Attachment"=>false));
    ...
    public function tabel_bahan_paparan()
    {
        ...
        // $get_sum_data = ($get_sum_data['rp_realisasi_keuangan'] / $get_sum_data['pagu_total']) * 100 ;

        $ratarata_realisasi_fisik	= $get_sum_data['realisasi_fisik'] / $total_opd;
        $ratarata_realisasi_keuangan	= ($get_sum_data['rp_realisasi_keuangan'] / $get_sum_data['pagu_total']) * 100 ;
		...
		$rk_dibawah_ratarata = [];
		$rk_diatas_ratarata = [];
		$rk_dibawah_ratarata_asisten_1 = [];
		$rk_diatas_ratarata_asisten_1 = [];
		$rk_dibawah_ratarata_asisten_2 = [];
		$rk_diatas_ratarata_asisten_2 = [];
		$rk_dibawah_ratarata_asisten_3 = [];
		$rk_diatas_ratarata_asisten_3 = [];

		$rf_dibawah_ratarata = [];
		$rf_diatas_ratarata = [];
		$rf_dibawah_ratarata_asisten_1 = [];
		$rf_diatas_ratarata_asisten_1 = [];
		$rf_dibawah_ratarata_asisten_2 = [];
		$rf_diatas_ratarata_asisten_2 = [];
		$rf_dibawah_ratarata_asisten_3 = [];
		$rf_diatas_ratarata_asisten_3 = [];
		...
		$total_rp_realisasi_keuangan+= ($satu->rp_realisasi_keuangan == '' ? 0 :  $satu->rp_realisasi_keuangan);
         ...
         $sort_rk = [
          	'rk'=>$satu->realisasi_keuangan == '' ? 0 :  $satu->realisasi_keuangan,
          	'skpd'=>$satu->nama_instansi,
          	'singkatan_skpd'=>$satu->singkatan_nama_instansi,
      		'tk'=>$satu->target_keuangan,
          	'dk'=>$dev_keu,
          ];
         $sort_rf = [
          ...
          if ($ratarata_realisasi_keuangan>$satu->realisasi_keuangan) {
             array_push($rk_dibawah_ratarata, $sort_rk	);
             array_push($rk_dibawah_ratarata_asisten_1, $sort_rk	);
          }else{
             array_push($rk_diatas_ratarata, $sort_rk);
             array_push($rk_diatas_ratarata_asisten_1, $sort_rk	);
          }

          if ($ratarata_realisasi_fisik>$satu->realisasi_fisik) {
             array_push($rf_dibawah_ratarata, $sort_rf	);
             array_push($rf_dibawah_ratarata_asisten_1, $sort_rf	);
          }else{
             array_push($rf_diatas_ratarata, $sort_rf);
             array_push($rf_diatas_ratarata_asisten_1, $sort_rf	);
          }
		...
		$total_rp_realisasi_keuangan +=($dua->rp_realisasi_keuangan == '' ? 0 :  $dua->rp_realisasi_keuangan);
         ...
         $sort_rk = [
          	'rk'=>$dua->realisasi_keuangan == '' ? 0 :  $dua->realisasi_keuangan,
          	'skpd'=>$dua->nama_instansi,
          	'singkatan_skpd'=>$dua->singkatan_nama_instansi,
      		'tk'=>$dua->target_keuangan,
          	'dk'=>$dev_keu,
          ];
         $sort_rf = [
          ...
          if ($ratarata_realisasi_keuangan>$dua->realisasi_keuangan) {
             array_push($rk_dibawah_ratarata, $sort_rk	);
             array_push($rk_dibawah_ratarata_asisten_2, $sort_rk	);
          }else{
             array_push($rk_diatas_ratarata, $sort_rk);
             array_push($rk_diatas_ratarata_asisten_2, $sort_rk	);
          }

          if ($ratarata_realisasi_fisik>$dua->realisasi_fisik) {
             array_push($rf_dibawah_ratarata, $sort_rf	);
             array_push($rf_dibawah_ratarata_asisten_2, $sort_rf	);
          }else{
             array_push($rf_diatas_ratarata, $sort_rf);
             array_push($rf_diatas_ratarata_asisten_2, $sort_rf	);
          }
         ...
         $sort_rk = [
          	'rk'=>$tiga->realisasi_keuangan == '' ? 0 :  $tiga->realisasi_keuangan,
          	'skpd'=>$tiga->nama_instansi,
          	'singkatan_skpd'=>$tiga->singkatan_nama_instansi,
      		'tk'=>$tiga->target_keuangan,
          	'dk'=>$dev_keu,
          ];
         $sort_rf = [
          ...
          if ($ratarata_realisasi_keuangan>$tiga->realisasi_keuangan) {
             array_push($rk_dibawah_ratarata, $sort_rk	);
          ...
          if ($ratarata_realisasi_fisik>$tiga->realisasi_fisik) {
             array_push($rf_dibawah_ratarata, $sort_rf	);
		 ...
		 $persentasi_rk_semua_diatas_ratarata = (count($rk_diatas_ratarata) / $total_opd) * 100;
		 $persentasi_rk_semua_dibawah_ratarata = (count($rk_dibawah_ratarata) / $total_opd) * 100;


		 $persentasi_rf_asisten_1_diatas_ratarata = (count($rf_diatas_ratarata_asisten_1) / count($asisten_1)) * 100;
		 $persentasi_rf_asisten_1_dibawah_ratarata = (count($rf_dibawah_ratarata_asisten_1) / count($asisten_1)) * 100;


		 $persentasi_rf_asisten_2_diatas_ratarata = (count($rf_diatas_ratarata_asisten_2) / count($asisten_2)) * 100;
		 $persentasi_rf_asisten_2_dibawah_ratarata = (count($rf_dibawah_ratarata_asisten_2) / count($asisten_2)) * 100;

		 $persentasi_rf_asisten_3_diatas_ratarata = (count($rf_diatas_ratarata_asisten_3) / count($asisten_3)) * 100;
		 $persentasi_rf_asisten_3_dibawah_ratarata = (count($rf_dibawah_ratarata_asisten_3) / count($asisten_3)) * 100;


		 $persentasi_rk_asisten_1_diatas_ratarata = (count($rk_diatas_ratarata_asisten_1) / count($asisten_1)) * 100;
		 $persentasi_rk_asisten_1_dibawah_ratarata = (count($rk_dibawah_ratarata_asisten_1) / count($asisten_1)) * 100;


		 $persentasi_rk_asisten_2_diatas_ratarata = (count($rk_diatas_ratarata_asisten_2) / count($asisten_2)) * 100;
		 $persentasi_rk_asisten_2_dibawah_ratarata = (count($rk_dibawah_ratarata_asisten_2) / count($asisten_2)) * 100;

		 $persentasi_rk_asisten_3_diatas_ratarata = (count($rk_diatas_ratarata_asisten_3) / count($asisten_3)) * 100;
		 $persentasi_rk_asisten_3_dibawah_ratarata = (count($rk_dibawah_ratarata_asisten_3) / count($asisten_3)) * 100;
		...
		$batasan = [
			'fisik'=>[
				'data'=>[
					'diatas_rata_rata'=>[
						'semua'=>$rf_diatas_ratarata,
						'asisten_1'=>$rf_diatas_ratarata_asisten_1,
						'asisten_2'=>$rf_diatas_ratarata_asisten_2,
						'asisten_3'=>$rf_diatas_ratarata_asisten_3,
					],
					'dibawah_rata_rata'=>[
						'semua'=>$rf_dibawah_ratarata,
						'asisten_1'=>$rf_dibawah_ratarata_asisten_1,
						'asisten_2'=>$rf_dibawah_ratarata_asisten_2,
						'asisten_3'=>$rf_dibawah_ratarata_asisten_3,
					],
				],
				'persentasi'=>[
					'diatas_rata_rata'=>[
						'semua'=>$persentasi_rf_semua_diatas_ratarata,
						'asisten_1'=>$persentasi_rf_asisten_1_diatas_ratarata,
						'asisten_2'=>$persentasi_rf_asisten_2_diatas_ratarata,
						'asisten_3'=>$persentasi_rf_asisten_3_diatas_ratarata,
					],
					'dibawah_rata_rata'=>[
						'semua'=>$persentasi_rf_semua_dibawah_ratarata,
						'asisten_1'=>$persentasi_rf_asisten_1_dibawah_ratarata,
						'asisten_2'=>$persentasi_rf_asisten_2_dibawah_ratarata,
						'asisten_3'=>$persentasi_rf_asisten_3_dibawah_ratarata,
					],
				],
			],
			'keuangan'=>[
				'data'=>[
					'diatas_rata_rata'=>[
						'semua'=>$rk_diatas_ratarata,
						'asisten_1'=>$rk_diatas_ratarata_asisten_1,
						'asisten_2'=>$rk_diatas_ratarata_asisten_2,
						'asisten_3'=>$rk_diatas_ratarata_asisten_3,
					],
					'dibawah_rata_rata'=>[
						'semua'=>$rk_dibawah_ratarata,
						'asisten_1'=>$rk_dibawah_ratarata_asisten_1,
						'asisten_2'=>$rk_dibawah_ratarata_asisten_2,
						'asisten_3'=>$rk_dibawah_ratarata_asisten_3,
					],
					'persentasi_semua'=>'',
				],
				'persentasi'=>[
					'diatas_rata_rata'=>[
						'semua'=>$persentasi_rk_semua_diatas_ratarata,
						'asisten_1'=>$persentasi_rk_asisten_1_diatas_ratarata,
						'asisten_2'=>$persentasi_rk_asisten_2_diatas_ratarata,
						'asisten_3'=>$persentasi_rk_asisten_3_diatas_ratarata,
					],
					'dibawah_rata_rata'=>[
						'semua'=>$persentasi_rk_semua_dibawah_ratarata,
						'asisten_1'=>$persentasi_rk_asisten_1_dibawah_ratarata,
						'asisten_2'=>$persentasi_rk_asisten_2_dibawah_ratarata,
						'asisten_3'=>$persentasi_rk_asisten_3_dibawah_ratarata,
					],
					'persentasi_semua'=>'',
				],
			],
			// 'ratarata_fisik'=>$ratarata_realisasi_fisik,
			'ratarata_keuangan'=>$ratarata_realisasi_keuangan,
		];
		 ...
		 $deviasi_fisik_opd = $this->rekap_asisten_model->realisasi_semua_opd('deviasi_fisik',$bulan,$kategori)->result();
		 $deviasi_keuangan_opd = $this->rekap_asisten_model->realisasi_semua_opd('deviasi_keuangan',$bulan,$kategori)->result();

		 $kumpul_realisasi_fisik = [];
		 $kumpul_realisasi_keuangan = [];
		 $kumpul_deviasi_fisik = [];
		 $kumpul_deviasi_keuangan = [];

		 $kumpul_realisasi_fisik_per_asisten_1 = [];
		 $kumpul_realisasi_fisik_per_asisten_2 = [];
		 $kumpul_realisasi_fisik_per_asisten_3 = [];
		 $kumpul_realisasi_fisik_inspektorat = [];

		 $total_nilai_fisik_1 = 0;
		 $total_nilai_fisik_2 = 0;
		 $total_nilai_fisik_3 = 0;




		 $kumpul_realisasi_keuangan_per_asisten_1 = [];
		 $kumpul_realisasi_keuangan_per_asisten_2 = [];
		 $kumpul_realisasi_keuangan_per_asisten_3 = [];
		$kumpul_realisasi_keuangan_inspektorat = [];

		 foreach ($realisasi_fisik_opd as $v) {
		 	$nilai_tf = $v->target_fisik == '' ? 0 : $v->target_fisik;
		 	$nilai_rf = $v->realisasi_fisik == '' ? 0 : $v->realisasi_fisik;
			$data_rf = [
	          ...
	          if ($v->id_parent==204) {
			 	...
			 	$total_nilai_fisik_1 += $nilai_rf;
		          $data_realisasi_fisik_per_asisten_1 = [
		       			'skpd'=>$v->nama_instansi,
		          ...
		          array_push($kumpul_realisasi_fisik_per_asisten_1, $data_realisasi_fisik_per_asisten_1);
	          }
	          elseif ($v->id_parent==205) {

			 	$total_nilai_fisik_2 += $nilai_rf;
		          $data_realisasi_fisik_per_asisten_2 = [
		       			'skpd'=>$v->nama_instansi,
		          ...
		          array_push($kumpul_realisasi_fisik_per_asisten_2, $data_realisasi_fisik_per_asisten_2);
	          }
	          elseif ($v->id_parent==206) {

			 	$total_nilai_fisik_3 += $nilai_rf;
		          $data_realisasi_fisik_per_asisten_3 = [
		       			'skpd'=>$v->nama_instansi,
		          ...
		          array_push($kumpul_realisasi_fisik_per_asisten_3, $data_realisasi_fisik_per_asisten_3);
	          }else{

		          $data_realisasi_fisik_inspektorat = [
		       			'skpd'=>$v->nama_instansi,
		          ...
		          array_push($kumpul_realisasi_fisik_inspektorat, $data_realisasi_fisik_inspektorat);
	          }

	          $kumpul_realisasi_fisik_per_asisten = [
	          	'asisten_1' => $kumpul_realisasi_fisik_per_asisten_1,
	          	'asisten_2' => $kumpul_realisasi_fisik_per_asisten_2,
	          	'asisten_3' => $kumpul_realisasi_fisik_per_asisten_3,
	          	'inspektorat' => $kumpul_realisasi_fisik_inspektorat,
	          ];
		 ...
		 foreach ($realisasi_keuangan_opd as $v) {
          ...
          $total_rp_r_keu += $v->rp_realisasi_keuangan;
          ...
          array_push($kumpul_realisasi_keuangan, $data_rk);
	          ...
	          if ($v->id_parent==204) {
	          		$data_realisasi_keuangan_per_asisten_1 = [
		       			'skpd'=>$v->nama_instansi,
			          	'singkatan_skpd'=>$v->singkatan_nama_instansi,
			          	'rk'=>$v->realisasi_keuangan == '' ? 0 : $v->realisasi_keuangan,
			          	'tk'=>$v->target_keuangan == '' ? 0 : $v->target_keuangan,
		          ...
		          array_push($kumpul_realisasi_keuangan_per_asisten_1, $data_realisasi_keuangan_per_asisten_1);

	          }
	          elseif ($v->id_parent==205) {
	          		$data_realisasi_keuangan_per_asisten_2 = [
		       			'skpd'=>$v->nama_instansi,
			          	'singkatan_skpd'=>$v->singkatan_nama_instansi,
			          	'rk'=>$v->realisasi_keuangan == '' ? 0 : $v->realisasi_keuangan,
			          	'tk'=>$v->target_keuangan == '' ? 0 : $v->target_keuangan,
		          ...
		          array_push($kumpul_realisasi_keuangan_per_asisten_2, $data_realisasi_keuangan_per_asisten_2);

	          }
	          elseif ($v->id_parent==206) {
	          		$data_realisasi_keuangan_per_asisten_3 = [
		       			'skpd'=>$v->nama_instansi,
			          	'singkatan_skpd'=>$v->singkatan_nama_instansi,
			          	'rk'=>$v->realisasi_keuangan == '' ? 0 : $v->realisasi_keuangan,
			          	'tk'=>$v->target_keuangan == '' ? 0 : $v->target_keuangan,
		          ...
		          array_push($kumpul_realisasi_keuangan_per_asisten_3, $data_realisasi_keuangan_per_asisten_3);

	          }else{

		          $data_realisasi_keuangan_inspektorat = [
		       			'skpd'=>$v->nama_instansi,
		          ...
		          array_push($kumpul_realisasi_keuangan_inspektorat, $data_realisasi_keuangan_inspektorat);
	          }
		  }

		   $kumpul_realisasi_keuangan_per_asisten = [
	          	'asisten_1' => $kumpul_realisasi_keuangan_per_asisten_1,
	          	'asisten_2' => $kumpul_realisasi_keuangan_per_asisten_2,
	          	'asisten_3' => $kumpul_realisasi_keuangan_per_asisten_3,
	          	'inspektorat' => $kumpul_realisasi_keuangan_inspektorat,
	          ];
		  $ratarata_realisasi_keuangan = ($total_rp_r_keu / $total_pagu) * 100;



		 foreach ($deviasi_fisik_opd as $v) {
		  $data_df = [
          	'skpd'=>$v->nama_instansi,
          	'singkatan_skpd'=>$v->singkatan_nama_instansi,
          	'df'=>$v->deviasi_fisik == '' ? 0 : $v->deviasi_fisik,
          ];
          array_push($kumpul_deviasi_fisik, $data_df);
		  }
		 foreach ($deviasi_keuangan_opd as $v) {
		  $data_dk = [
          	'skpd'=>$v->nama_instansi,
          	'singkatan_skpd'=>$v->singkatan_nama_instansi,
          	'dk'=>$v->deviasi_keuangan == '' ? 0 : $v->deviasi_keuangan,
          ];
          array_push($kumpul_deviasi_keuangan, $data_dk);
		  }
		...
		$data['eeeeeeeee']  = $batasan;


		$data['get_sum_data']    = $get_sum_data;
        $data['grafik_realisasi_fisik']    = $kumpul_realisasi_fisik;
        $data['grafik_realisasi_fisik_asisten']    = $kumpul_realisasi_fisik_per_asisten;
        $data['ratarata_realisasi_fisik']    = $ratarata_realisasi_fisik;


        $data['grafik_realisasi_keuangan']    = $kumpul_realisasi_keuangan;
        $data['grafik_realisasi_keuangan_asisten']    = $kumpul_realisasi_keuangan_per_asisten;
        $data['ratarata_realisasi_keuangan']    = $ratarata_realisasi_keuangan;

        $data['grafik_deviasi_fisik']    = $kumpul_deviasi_fisik;
        $data['grafik_deviasi_keuangan']    = $kumpul_deviasi_keuangan;
    ...
    public function tabel_rekap_semua_opd_detail()
    {
        $breadcrumbs     = $this->breadcrumbs;
        $master_paket   = $this->master_paket_model;
        $data_apbd_model   = $this->data_apbd_model;
        ...
        $laporan_opd = $this->rekap_asisten_model->get_rfk_opd($bulan, $tahap, $tahun, $kategori)->result_array();
		

		$data['skpd']    = $laporan_opd;
        $data['tahun']                      = $tahun;
        $data['bulan']                      = $bulan;
        $data['tahap']                      = $tahap;
        $data['kategori']                      = $kategori;
    	...
    	$data['nama_instansi']				=$this->db->query("SELECT nama_instansi from master_instansi where id_instansi='$id_instansi'")->row()->nama_instansi;
    	$data['controller']				=$this->router->fetch_class();
        $page                               = 'laporan/tabel/tabel_rekap_semua_opd_detail/tabel_rekap_semua_opd_detail';
        $data['link']                       = $this->router->fetch_method();
        $data['menu']                       = $this->load->view('layout/menu', $data, true);
        $data['modal']                      = $this->load->view('laporan/tabel/tabel_rekap_semua_opd_detail/modal', $data, true);
        $this->load->view($page, $data);
    }
...
$excel->getActiveSheet()->getStyle('E'.$row_statistika_d_10.':J'.$row_statistika_total_data)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); 
    ...
    public function daftar_kpa()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $list_kpa = $this->db->query("SELECT * from v_kpa where id_instansi = '$id_instansi' and id_kedudukan='2'");

             if ($list_kpa->num_rows()>0) {
	             $output['status'] =true;
	             $output['data'] = $list_kpa->result_array();
             	# code...
             }else{

	             $output['status'] =false;
	             $output['data'] = [];
	             $output['message'] = 'Data KPA tidak ada';
             }
    ...
    public function daftar_instansi_pembantu()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
            ...
            $id_instansi = sbe_crypt($this->input->post('id_instansi'), 'D');
            $list_instansi_pembantu = $this->db->query("SELECT id_instansi_pembantu_teknis, nama_instansi_teknis from instansi_pembantu_teknis where id_instansi = '$id_instansi' and is_active='1'");

             if ($list_instansi_pembantu->num_rows()>0) {
	             $output['status'] =true;
	             $output['data'] = $list_instansi_pembantu->result_array();
             	# code...
             }else{

	             $output['status'] =false;
	             $output['data'] = [];
	             $output['message'] = 'Instansi pembantu tidak ada';
             }
    ...
    public function daftar_pptk()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $list_pptk = $this->db->query("SELECT * from v_pptk where id_instansi = '$id_instansi' and id_kedudukan='3' and id_user_top_parent='$id_kpa'");

             if ($list_pptk->num_rows()>0) {
	             $output['status'] =true;
	             $output['data'] = $list_pptk->result_array();
             	# code...
             }else{

	             $output['status'] =false;
	             $output['data'] = [];
	             $output['message'] = 'Data PPTK tidak ada';
             }
