...
class Rekap_asisten_model extends CI_Model
{
  ...
  public function get_rfk_opd($bulan, $tahap, $tahun, $kategori)
  {
  ...
  public function get_sum_data($bulan)
  {
    $kategori = 'Akumulasi';
    $tahap        = $this->input->get('tahap');
    $tahun        = $this->input->get('tahun');
    ...
    $query  = $this->db->query("SELECT 
                                  sum(g.target_fisik_akumulasi) as target_fisik,
                   sum(g.realisasi_fisik_akumulasi) as realisasi_fisik,
                   sum(g.target_keuangan_akumulasi) as target_keuangan,
                   sum(g.realisasi_keuangan_akumulasi) as realisasi_keuangan,
                   sum(g.rp_target_keuangan_akumulasi) as rp_target_keuangan,
                   sum(g.rp_realisasi_keuangan_akumulasi) as rp_realisasi_keuangan,

                                    sum(g.pagu_total) as pagu_total
                                FROM grafik g  WHERE
                                   g.kode_tahap = '$tahap'
                                  and g.tahun = '$tahun'
                                  AND g.bulan = '$bulan'
    ...
    // if ($cara_hitung=='Akuntansi') {
      if ($kategori=='Akumulasi') {
         $query_sql = 'g.target_fisik_akumulasi as target_fisik,
                   ...
                   (g.realisasi_fisik_akumulasi - g.target_fisik_akumulasi) as deviasi_fisik
                   ';
                     ...
                     elseif ($realisasi=='keuangan') {
                        $order_by = "ORDER by g.realisasi_keuangan_akumulasi desc";
                      }
                     elseif ($realisasi=='deviasi_fisik') {
                        $order_by = "ORDER by (g.realisasi_fisik_akumulasi - g.target_fisik_akumulasi) desc";
                      }
                      else{
                        $order_by = "ORDER by (g.realisasi_keuangan_akumulasi - g.target_keuangan_akumulasi) desc";
         ...
         $query_sql = 'g.target_fisik_bulanan as target_fisik,
                   ...
                   g.rp_target_keuangan_bulanan as rp_target_keuangan,
                   g.rp_realisasi_keuangan_bulanan as rp_realisasi_keuangan,
                    (g.realisasi_fisik_bulanan - g.target_fisik_bulanan) as deviasi_fisik,
                   ...
                   ';
                    ...
                    elseif ($realisasi=='deviasi_fisik') {
                        $order_by = "ORDER by (g.realisasi_fisik_bulanan - g.target_fisik_bulanan) desc";
                      }
                      else{
                        $order_by = "ORDER by (g.realisasi_keuangan_bulanan - g.target_keuangan_bulanan) desc";
