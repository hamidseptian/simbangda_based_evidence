		...
		$.ajax(
        {
            ...
            success : function(data)
            {
                ...
                if(data.status == true)
                {
					...
					{
						$('#sub_kegiatan').append(
											'<option value="'+ v.rekening_sub_kegiatan +'_'+ v.rekening_kegiatan +'_'+ v.rekening_program +'_'+ v.rekening_bu +'_'+ v.kode_tahap +'">['+v.nama_tahap+'] '+ v.rekening_sub_kegiatan + ' - ' + v.nama_sub_kegiatan + v.keterangan +'</option>'
										 	 );
					});
				}else{
					$('.select-kegiatan').hide();
