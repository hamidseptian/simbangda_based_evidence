 ...
 public function rules_save_user()
	...
	public function list_kegiatan()
	{
		if (!$this->input->is_ajax_request()) {
			...
			$tahun 			= tahun_anggaran();
			if ($tahap==4) {
				$kegiatan = $this->db->query("SELECT * FROM v_sub_kegiatan_apbd vska where vska.id_instansi='$id_instansi' and vska.status='1'and vska.tahun='$tahun' and vska.kode_rekening_sub_kegiatan NOT IN 
                     (SELECT kode_rekening_sub_kegiatan FROM users_sub_kegiatan usk
                     join master_users mu on usk.id_user = mu.id_user and mu.id_kedudukan=3 and usk.id_instansi=mu.id_instansi
                     join sub_instansi si on mu.id_sub_instansi = si.id_sub_instansi and si.id_instansi = '$id_instansi' and si.status=1
                      WHERE usk.id_instansi='$id_instansi' and usk.tahun_anggaran='$tahun'  and usk.status=1) 
                    order by vska.kode_rekening_sub_kegiatan  asc");
			}else{
				$kegiatan = $this->db->query("SELECT * FROM v_sub_kegiatan_apbd vska where vska.id_instansi='$id_instansi' and vska.kode_tahap='$tahap'and vska.tahun='$tahun' and vska.kode_rekening_sub_kegiatan NOT IN 
                   (SELECT kode_rekening_sub_kegiatan FROM users_sub_kegiatan usk
                     join master_users mu on usk.id_user = mu.id_user and mu.id_kedudukan=3 and usk.id_instansi=mu.id_instansi
                     join sub_instansi si on mu.id_sub_instansi = si.id_sub_instansi and si.id_instansi = '$id_instansi' and si.status=1
                      WHERE usk.id_instansi='$id_instansi' and usk.tahun_anggaran='$tahun'  and usk.kode_tahap=2) 
                       order by vska.kode_rekening_sub_kegiatan asc");
			}
