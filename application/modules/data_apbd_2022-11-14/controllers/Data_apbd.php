...
class Data_apbd extends MY_Controller
{
    ...
    public function sub_kegiatan($input_by)
    {
        ...
        $breadcrumbs->render();
        $data['dropdown_option']                      = [
            ['tipe'=>'link', 'caption'=>'Kembali', 'fa'=>'fa fa-arrow-left', 'onclick'=>'data_apbd/sub_kegiatan/all', 'elemen_tambahan'=>'data-toggle="tooltip" title="Kembali"'],
    ...
    public function sub_kegiatan_apbd_instansi_gabungan($input_by)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                $row[]  =  '<span style="float:right">'.number_format($pagu,0,'','.').'</span>';
                $row[]  = $lists->input_by;
                $row[]  = $status_sub_kegiatan[$lists->status];
    ...
    public function pengalihan_apbd_sub_kegiatan_apbd_instansi_gabungan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                $onclick_tetap = "tidak_berubah_sub_kegiatan_instansi_apbd_awal_dan_perubahan('".$lists->id_sub_kegiatan_instansi."','".$nama_sub_kegiatan."')";
