...
class Realisasi extends MY_Controller
{
    ...
    public function fisik()
    {
        ...
        $data['total_paket']           = $master_paket->total_paket_pekerjaan();
        $data['total_paket_rutin']     = $master_paket->total_jenis_paket_pekerjaan('RUTIN');
        $data['total_paket_swakelola'] = $master_paket->total_jenis_paket_pekerjaan('SWAKELOLA');
        $data['total_paket_penyedia']  = $master_paket->total_jenis_paket_pekerjaan('PENYEDIA');
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($kegiatan->num_rows() > 0) :
                ...
                foreach ($kegiatan->result() as $key => $value) {
                    ...
                    $output['data'][$key]['status_sub_kegiatan'] = $caption_status_pengalihan[$status];
            ...
            foreach ($list as $lists) {
                ...
                if ($lists->jenis_paket=='RUTIN') {
                    ...
                    $show_nilai_paket_rutin = $nilai_paket_rutin > 0 ? $nilai_paket_rutin : 0 ;
                     ...
                     $row[]  = '<button class="btn btn-info btn-sm" onclick="upload_dokumen(' . "'" . $lists->id_instansi . "','" . 'null' . "','" . $lists->id_pptk . "','" . $lists->id_paket_pekerjaan . "','" . $lists->kode_rekening_sub_kegiatan . "','" . $lists->nama_paket . "','" . $lists->jenis_paket . "','" . $lists->id_metode . "','" . $izin_input_pptk . "','" . $id_kedudukan . "'" . ')"><i class="fas fa-upload"></i></button>';
    ...
    public function list_dokumen()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($jenis_paket=='RUTIN') {
                $dokumen = $this->db->query("SELECT * from master_bobot where jenis_paket='$jenis_paket' and nama_dokumen not in (SELECT dokumen from realisasi_fisik where id_paket_pekerjaan='$id_paket_pekerjaan')");
            }else{
                ...
                foreach ($dokumen_realisasi->result() as $key => $value) {
                    ...
                    if (jadwal_rfk()['aktif']==0) {
                        $tombol_edit_evidence =    ' <a href="#" class="btn btn-danger btn-sm" data-toggle="tooltip" title="upload ulang evidence" onclick="upload_berakhir()"><i class="metismenu-icon fas fa-file-signature"></i></a>';
                            $tombol_hapus_evidence = ' <button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Lihat Evidence evidence" target="_blank" onclick="upload_berakhir()"><i class="metismenu-icon fas fa-trash"></i></button>';
                    }else{
    ...
    public function cek_rk()
    {
        ...
        $data['title']       = "Cek Realisasi Keuangan Ganda";
        $data['icon']        = "metismenu-icon fas fa-money-bill";
        $data['description'] = "Menampilkan Realisasi Keuangan yang datanya terinput double";
        $data['breadcrumbs'] = $breadcrumbs->render();
        $tahap = $this->input->get('kode_tahap');
        $id_instansi = id_instansi();
        $tahun = $this->input->get('tahun');
        $krsk = $this->input->get('krsk');
        $page                = 'realisasi/keuangan/cek_rk_ganda';
        $data['tahap']        = pilihan_nama_tahapan($tahap);
        $data['tahun']        = $tahun;

        $sub_kegiatan = $this->db->query("SELECT * from v_sub_kegiatan_apbd where tahun= '$tahun' and kode_tahap='$tahap' and id_instansi='$id_instansi' and kode_rekening_sub_kegiatan='$krsk'")->row_array();
        $nama_sub_kegiatan = $sub_kegiatan['kategori']=='Sub Kegiatan SKPD' ? $sub_kegiatan['nama_sub_kegiatan'] : $sub_kegiatan['nama_sub_kegiatan'].'<br>'.$sub_kegiatan['jenis_sub_kegiatan'].' - '.$sub_kegiatan['keterangan'];
        $rk = $this->db->query("SELECT * from realisasi_keuangan where tahun= '$tahun' and kode_tahap='$tahap' and id_instansi='$id_instansi' and kode_sub_kegiatan='$krsk' order by bulan asc");
        $data['rk']        = $rk->result_array();
        $data['sub_kegiatan']        = $nama_sub_kegiatan;
        $data['link']        = $this->router->fetch_method();
        $data['menu']        = $this->load->view('layout/menu', $data, true);
        $data['extra_css']   = $this->load->view('realisasi/keuangan/css', $data, true);
        $data['extra_js']    = $this->load->view('realisasi/keuangan/js', $data, true);
    ...
    public function cek_rk_kk()
    {
        ...
        $data['breadcrumbs'] = $breadcrumbs->render();
        $tahap = $this->input->get('kode_tahap');
        $id_instansi = sbe_crypt($this->input->get('id_instansi'),'D');
        $tahun = $this->input->get('tahun');
        $krsk = $this->input->get('krsk');
        $page                = 'realisasi/fisik_keuangan/cek_rk_kk_ganda';
        $data['tahap']        = pilihan_nama_tahapan($tahap);
        $data['tahun']        = $tahun;

        $instansi = $this->db->query("SELECT nama_instansi from master_instansi_kab_kota where id_instansi= '$id_instansi'")->row_array();
      
        $rk = $this->db->query("SELECT * from realisasi_fisik_keuangan_kab_kota where tahun= '$tahun' and kode_tahap='$tahap' and id_instansi='$id_instansi' order by bulan asc");
    ...
    public function hapus_rk_perbulan($id_realisasi_keuangan)
    {
        $id_rk=sbe_crypt($id_realisasi_keuangan, 'D');
        $tahap = $this->input->get('kode_tahap');
        $id_instansi = id_instansi();
        $tahun = $this->input->get('tahun');
        $krsk = $this->input->get('krsk');
        $rk = $this->db->query("DELETE from realisasi_keuangan where id_realisasi_keuangan= '$id_rk' ");
        $this->session->set_flashdata('pesan','<div class="alert alert-info">Realisasi dihapus</div>');
        redirect('realisasi/cek_rk?krsk='.$krsk.'&tahun='.$tahun.'&kode_tahap='.$tahap);
    }
    public function hapus_rk_kk_perbulan($id_realisasi_keuangan)
    {
        $id_rk=sbe_crypt($id_realisasi_keuangan, 'D');
        $tahap = $this->input->get('kode_tahap');
        $id_instansi = id_instansi();
        ...
        $rk = $this->db->query("DELETE from realisasi_fisik_keuangan_kab_kota where id_realisasi_fisik_keuangan_kab_kota= '$id_rk' ");
        $this->session->set_flashdata('pesan','<div class="alert alert-info">Realisasi dihapus</div>');
        redirect('realisasi/cek_rk_kk?id_instansi='.$id_instansi.'&tahun='.$tahun.'&kode_tahap='.$tahap);
    }
    ...
    public function data_realisasi($id_instansi)
    {
        ...
        @$output['persen']['bo']  = $anggaran==0 ? 0 : ROUND($bo =='' ? 0 : $bo / $anggaran * 100, 2);
        @$output['persen']['bm']  = $anggaran==0 ? 0 : ROUND($bm =='' ? 0 : $bm / $anggaran * 100, 2);
        @$output['persen']['btt'] = $anggaran==0 ? 0 : ROUND($btt =='' ? 0 : $btt / $anggaran * 100, 2);
        @$output['persen']['bt']  = $anggaran==0 ? 0 : ROUND($bt =='' ? 0 : $bt / $anggaran * 100, 2);
        @$output['persen']['total']  = $anggaran==0 ? 0 : ROUND($total =='' ? 0 : $total / $anggaran * 100, 2);
    ...
    public function data_instansi_kab_kota()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $tahun_anggaran = tahun_anggaran();
                $id_kota = $this->session->userdata('id_kota') ;    
                $where          = ['kategori'=>'OPD', 'id_kota'=>$id_kota, 'is_active'=>1];
            ...
            foreach ($list as $lists) {
                ...
                $q_direalisasikan = $this->db->query("SELECT * from realisasi_fisik_keuangan_kab_kota where id_instansi='$id_instansi' and kode_tahap='$tahap' and tahun = '$tahun_anggaran'")->num_rows();
                $row    = [];
                $row[]     = $no;
               ...
               //  // $row[]  = number_format($total_realisasi);
                ...
                $tombol_copy = ' <button class="btn btn-outline-info btn-xs"  title="Copy Realisasi Fisik & Keuangan  APBD AWAL Instansi '.$lists->nama_instansi.'"  onclick="copy_realisasi_apbd_awal_kab_kota('."'".sbe_crypt($lists->id_instansi, 'E')."'".','.$tahap.', '.$tahun_anggaran.', '."'".$lists->nama_instansi."'".')"><i class="fas fa-copy"></i></button>';
                $tombol_hapus = ' <button class="btn btn-danger btn-xs"  title="Hapus Realisasi '.nama_tahapan($tahap).' Instansi '.$lists->nama_instansi.'"  onclick="hapus_rfk_kab_kota('."'".sbe_crypt($lists->id_instansi, 'E')."'".','.$tahap.', '."'".$lists->nama_instansi."','".$tahap."'".')"><i class="fas fa-trash"></i></button>';
                 ...
                 @$persen_lra = $pagu_total > 0 ? $lra / $pagu_total * 100 : 0;

                $row[]  = $set_lra;
                $row[]  = round($persen_lra,2);
                $row[]  = $pagu_total > 0 && $q_direalisasikan == 0 && $tahap==4 ? $tombol_copy : $tombol_hapus;
            ...
            $tahun_anggaran = tahun_anggaran();
                ...
                tahun = '$tahun_anggaran'
                    ...
                    $pagu = $anggaran->row_array();
                       ...
                       $cek_data  =$this->db->query("SELECT 
                         id_realisasi_fisik_keuangan_kab_kota   
                     from realisasi_fisik_keuangan_kab_kota   
                        where
                        kode_tahap='$tahap' and
                        tahun='$tahun_anggaran' and
                        id_instansi='$id_instansi' and
                        bulan = '$bulan'
                ")->num_rows();
                    ...
                    $output['data'][$key]['id_instansi']      = sbe_crypt($id_instansi,'E');
                    $output['data'][$key]['kode_tahap']      = $tahap;
                    $output['data'][$key]['tahun']      = $tahun_anggaran;
                    $output['data'][$key]['bulan']      = $bulan;
                    if ($jenis_realisasi=='bo') {
                        $output['data'][$key]['bo_bp']      = $realisasi['bo_bp']=='' ? 0 : $realisasi['bo_bp'];
                        $output['data'][$key]['bo_bbj']      = $realisasi['bo_bbj']=='' ? 0 : $realisasi['bo_bbj'];
 ...
 public function copy_realisasi_apbd_awal_kab_kota()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $where = ['id_instansi'=>$id_instansi, 'kode_tahap'=>'2','tahun'=>$tahun];
 ...
 public function get_anggaran_instansi_kab_kota()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $jenis_realisasi =  $this->input->post('jenis');
            $tahap = tahapan_apbd() ;//config_kab_kota()->tahapan_apbd;
            $tahun = tahun_anggaran() ;//config_kab_kota()->tahapan_apbd;
            $id_instansi = $this->input->post('id_instansi');
            
            $pagu = $this->db->query("SELECT * from v_instansi_kab_kota where id_instansi='$id_instansi' and kode_tahap='$tahap' and tahun='$tahun'")->row();
            $direalisasikan = $this->db->query("SELECT * from realisasi_fisik_keuangan_kab_kota where id_instansi='$id_instansi' and kode_tahap='$tahap' and tahun='$tahun'")->num_rows();
 ...
 public function hapus_r_keu_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $kode_program = $this->input->post('kode_program');
            $kode_bidang_urusan = $this->input->post('kode_bidang_urusan');
            $id_realisasi_keuangan = sbe_crypt($this->input->post('id_realisasi_keuangan'),'D');
            $tahap = $this->input->post('tahap');
            $tahun = tahun_anggaran();
            ...
            if ($bulan_dihapus=='semua') {
                ...
                $where_rk = [
                    ...
                    'tahun'=>tahun_anggaran(),
                    'bulan'=>$bulan_dihapus,
                    'id_realisasi_keuangan'=>$id_realisasi_keuangan,
                    'id_instansi' => id_instansi(),
                ];
    ...
    public function nilai_realisasi_fisik($id_instansi)
    {
        ...
        foreach ($kegiatan as $key => $value) {
            ...
            for ($i = 1; $i <= bulan_aktif(); $i++) {
                ...
                @$ratarata_fisik = $total_paket == 0 ? 0: $total_fisik_perkegiatan / $total_paket;
