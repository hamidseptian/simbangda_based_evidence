	                ...
	                if (data.status == true) {
						...
						$.each(data.data, function(k, v) {
                            ...
                            total_realisasi_fisik +=total_fisik;
                             ...
                             // tombol_hapus = '<br><a href="javascript:void(0)" class="btn btn-outline-danger btn-xs"  title="cek realisasi pada SKPD '+ '???' +' pada bulan '+bulan(v.bulan)+'" onclick="Swal.fire(`Error`,`Fitur ini belum bisa digunakan`,`error`)" >Terinput Ganda</a> ';
                             tombol_hapus = '<br><a href="'+baseUrl('realisasi/cek_rk_kk?id_instansi=')+ v.id_instansi + '&tahun='+ v.tahun + '&kode_tahap=' + v.kode_tahap + '" class="btn btn-outline-danger btn-xs"  title="cek realisasi pada SKPD '+ '???????????' +' pada bulan '+bulan(v.bulan)+'" target="_blank" >Terinput Ganda</a> ';
...
function copy_realisasi_apbd_awal_kab_kota(id_instansi, tahap, tahun,  nama_instansi)
    {      
