...
<script>
	...
	$(document).ready(function()
	{
		select2();
	   	...
	   	else if (fetch_method_pemilihan_data_apbd=='pergeseran') {
		   	show_sub_kegiatan_apbd_instansi_gabungan_pergeseran();
		   	// alert(0);
	...
	function select2()
	{
		...
		$('#id_instansi_teknis').select2({
			placeholder: "Pilih Instansi Teknis",
			allowClear: false,
			width: 'style',
			theme: 'bootstrap4'
		});
	...
	function input_anggaran(kode_rekening_sub_kegiatan,tahap, tahun, pengelompokan)
	{
	  ...
	  $('#form_anggaran_sub_kegiatan')[0].reset();
		...
		$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan').attr('style','');
		$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan_pergeseran').attr('style','display:none');
    	}else{
    		if (pagu_berubah=='') {

				$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan').attr('style','');
				$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan_pergeseran').attr('style','');
				$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan_pergeseran').html('Alihkan Pagu Pergeseran');
    		}else{
    			// input_anggaran_pergeseran(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pengelompokan, lakukan_pergeseran, pagu_berubah, pagu_berubah_pada_pergeseran_ke)
...
function simpanedit_pergeseran_ke(){
	...
	function input_anggaran_pergeseran(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pergeseran_ke, pengelompokan)
	{
	  ...
	  $('#form_anggaran_sub_kegiatan')[0].reset();
		...
		$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan').attr('style','display:none');
		$('#modal_input_anggaran').find('#btn_save_anggaran_sub_kegiatan_pergeseran').attr('style','');
		...
		$.ajax(
        {
            url     : baseUrl('data_apbd/get_anggaran_sub_kegiatan_pergeseran/'),
            dataType: 'JSON',
            type    : 'POST',
            data    : { 
            	kode_rekening_sub_kegiatan : kode_rekening_sub_kegiatan,
	...
	function save_anggaran_sub_kegiatan_pergeseran()
	{
		let id_pptk 	= $('#id_pptk').val();
		//let id_table 	= $('#kode_rekening_kegiatan').val().split('.').join('-');
		...
		$.ajax({
			url: baseUrl('data_apbd/save_anggaran_sub_kegiatan_pergeseran'),
			type: 'POST',
			dataType: 'JSON',
			data: $('#form_anggaran_sub_kegiatan').serialize(),
			success: function (datanya)
...
function get_target(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pagu)
	{	
    	...
    	$('#modal-target').find('#pagu').val(show_pagu);
		...
		$.ajax(
        {
            ...
            {	$('#modal-target').find('.kode_sub_kegiatan').html(data.kode_sub_kegiatan);
		    	$('#modal-target').find('#pagu_sub_kegiatan').html(convert_to_rupiah(show_pagu));
						...
						$.each(data.data, function(k, v) {
							if (data.kunci==0) {
								$('#target-apbd').append(
									...
									'<td style="text-align: right;"><a href="javascript:void(0)"  class="text-danger" onclick="'+data.alert_kunci+'">' + v.t_fisik_bulanan + '</a></td>' +
									...
									'<td style="text-align: right;">' + '<a href="javascript:void(0)"  class="text-danger" onclick="'+data.alert_kunci+'">' + convert_to_rupiah(v.t_keuangan_bulanan) + '</a>'  + '</td>' +
								...
								if (show_pagu==0) {
									a_href_tk =  '<a href="javascript:void(0)" onclick="Swal.fire('+ "'Tidak Diizinkan','Mohon maaf, anda tidak bisa  mengubah target keuangan karena tidak ada pagu','error'" +')" class="text-danger">' + convert_to_rupiah(v.t_keuangan_bulanan) + '</a>';

									td_persen_keu = '<td style="text-align: right;">0</td><td style="text-align: right;">0</td>' ;
								}else{
									a_href_tk =  '<a href="javascript:void(0)" id="target-fisik" kode_sub_kegiatan="' + kode_rekening_sub_kegiatan + '" kode_bidang_urusan="' + kode_bidang_urusan + '" kode_program="' + kode_program + '" kode_kegiatan="' + kode_kegiatan + '" pagu="' + pagu + '"  tahap="' + tahap + '", tahun="' + tahun + '" pk="' + v.id + '" class="edit" data-type="text" onclick="edit_target_keuangan(this)">' + convert_to_rupiah(v.t_keuangan_bulanan) + '</a>';

									td_persen_keu = '<td style="text-align: right;">' + ((v.t_keuangan_bulanan / pagu) * 100).toFixed(2) + '</td>' +
									'<td style="text-align: right;">' + ((v.t_keuangan / pagu) * 100).toFixed(2) + '</td>' ;
								}
								$('#target-apbd').append(
									'<tr>' +
									'<td style="text-align: right;">' + (k + 1) + '</td>' +
									'<td style="text-align: right;">' + bulan(v.bulan) + '</td>' +
									'<td style="text-align: right;"><a href="javascript:void(0)" id="target-fisik" kode_sub_kegiatan="' + kode_rekening_sub_kegiatan + '" kode_bidang_urusan="' + kode_bidang_urusan + '" kode_program="' + kode_program + '" kode_kegiatan="' + kode_kegiatan + '" pagu="' + pagu + '"  tahap="' + tahap + '", tahun="' + tahun + '" pk="' + v.id + '" class="edit" data-type="text" onclick="edit_target_fisik(this)">' + v.t_fisik_bulanan + '</a></td>' +

									'<td style="text-align: right;">' + v.t_fisik + '</td>' +
									td_persen_keu + 
									'<td style="text-align: right;">' + a_href_tk + '</td>' +
									'<td>' + convert_to_rupiah(v.t_keuangan) + '</td>' +
									'<td>' + v.input_by + '</td>' +
									'</tr>'
								);
...
function get_target_pergeseran(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pagu, pergeseran_ke)
	{	
		$('#modal-target').modal('show');
		$('.form-control').removeClass('is-valid')
						  .removeClass('is-invalid');
		...
		$.ajax(
        {
            url     : baseUrl('data_apbd/get_target_pergeseran/'),
            dataType: 'JSON',
            type    : 'POST',
            data    : { 
            	kode_rekening_sub_kegiatan : kode_rekening_sub_kegiatan,
            ...
            {	$('#modal-target').find('.kode_sub_kegiatan').html(data.kode_sub_kegiatan);
            	...
            	$('#modal-target').find('#nama_tahapan').html(data.nama_tahapan);
            	if (data.totaldata == 0) {
            		get_target_pergeseran(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pagu, pergeseran_ke);
            	}else{
	                ...
	                if (data.status == true) {
						$.each(data.data, function(k, v) {
							$('#target-apbd').append(
								...
								'<td style="text-align: right;">' + (k + 1) + '</td>' +
								'<td style="text-align: right;">' + bulan(v.bulan) + '</td>' +
								'<td style="text-align: right;"><a href="#" id="target-fisik" kode_sub_kegiatan="' + kode_rekening_sub_kegiatan + '" kode_bidang_urusan="' + kode_bidang_urusan + '" kode_program="' + kode_program + '" kode_kegiatan="' + kode_kegiatan + '" pagu="' + pagu + '"  tahap="' + tahap + '" tahun="' + tahun + '" pergeseran_ke="' + pergeseran_ke + '" pk="' + v.id + '" class="edit" data-type="text" onclick="edit_target_fisik_pergeseran(this)">' + v.t_fisik_bulanan + '</a></td>' +
	...
	function get_sumber_dana(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pagu) {
		...
		var show_pagu = pagu=='0'? 0: pagu ; 
		if (show_pagu==0) {
						$('#modal-sumber_dana').find('#tombol_simpan_sd').attr('onclick','Swal.fire('+ "'Tidak Diizinkan','Mohon maaf, anda tidak bisa  mengubah sumber dana karena tidak ada pagu','error'" +')');
						$('#modal-sumber_dana').find('#tombol_simpan_sd').attr('class','btn btn-outline-danger');
		}else{
						$('#modal-sumber_dana').find('#tombol_simpan_sd').attr('onclick','save_sumber_dana()');
						$('#modal-sumber_dana').find('#tombol_simpan_sd').attr('class','btn btn-info');
		}
		...
		$.ajax({
			...
			success: function(data) {
				$('#modal-sumber_dana').find('#pagu_sub_kegiatan').html(convert_to_rupiah(show_pagu));
				...
				if (data.status == true) {
					...
					$('#id_jenis_sumber_dana').val(data.data.id_jenis_sumber_dana).change();

					if (data.data.id_jenis_sumber_dana=='Lainnya') {
	...
	function input_ski_teknis(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap,  pagu) {
    	...
    	$('#modal-ski_teknis').find('#id_instansi_teknis').val('').change();
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data.banyak_instansi_teknis > 0 ) {

					$('#form_pilih_instansi_teknis').html(`
			    		<div class="input-group">
			                <div class="custom-file">
			                    <select name="id_instansi_teknis" id="id_instansi_teknis" class="form-control" style="width:100%">
			                    </select>
			                </div>
			                <div class="input-group-append">
			                     <button type="button" class="btn btn-primary" onclick="save_ski_teknis()">Tambahkan</button>
			                </div>
			            </div>
			            `);
					...
					$('#form_pilih_instansi_teknis').html(`
			    		<div class="alert alert-danger">
			             Sub kegiatan `+data.nama_sub_kegiatan+` sudah ada pada semua   instansi pembantu teknis
			            </div>
			            `);
				}
				...
				if (data.banyak_ski_teknis >0 ) {
					$('#modal-ski_teknis').find('#daftar_instansi_teknis').append(`
						 <table class="table table-bordered" id="list_instansi_teknis">
                                    <tr>
                                        <td>No</td>
					...
					$('#modal-ski_teknis').find('#list_instansi_teknis').append(`
                                 <tr>
                                        ...
                                        <td> `+v.jenis_sub_kegiatan+`</td>
                                        <td> `+v.keterangan+`</td>
                                        <td> ` + `<button type="button" class="btn btn-outline-danger btn-xs"  title=""  onclick="hapus_sub_kegiatan_instansi('`+v.kode_sub_kegiatan+`','`+v.kode_kegiatan+`','`+v.kode_program+`','`+v.kode_tahap+`','`+v.tahun+`','instansi_teknis')"><i class="fas fa-times"></i></button>` +`</td>
                                    </tr>`);
					...
					$('#modal-ski_teknis').find('#daftar_instansi_teknis').html('<div class="alert alert-info">Belum ada data</div>')
				}
			},
			error : function(){
		...
		$.ajax({
			...
			success: function (data)
			{
				...
				if(data.success == true)
				{
					...
					var tahap = data.data.tahap;
					var kode_bidang_urusan = data.data.kode_bidang_urusan;
					var kode_kegiatan = data.data.kode_kegiatan;
					var kode_program = data.data.kode_program;
					var kode_rekening_sub_kegiatan = data.data.kode_sub_kegiatan;

					Swal.fire('Disimpan','Sub Kegiatan Unit Pelaksana Disimpan','success');

					input_ski_teknis(kode_rekening_sub_kegiatan, kode_kegiatan, kode_program, tahap,0);
				   	$('#table-pilihan-sub-kegiatan-apbd').DataTable().ajax.reload(null, false);
					// $('#closemodal').click();
...
function edit_target_fisik(x) {
		...
		$(x).editable({
			...
			pk: id,
			savenochange: true,
			url: baseUrl('data_apbd/update_target_fisik/' + kode_sub_kegiatan + '/' + tahap),
			success: function(c) {
				get_target(kode_sub_kegiatan, kode_kegiatan, kode_program, tahap, tahun, kode_bidang_urusan, pagu)
			},
		});
	}
function edit_target_fisik_pergeseran(x) {
		$.fn.editableform.buttons = '<button type="submit" class="btn btn-primary btn-xs editable-submit">OK</button>' +
			'<button type="button" class="btn btn-default btn-xs editable-cancel">Batal</button>';
		...
		$(x).editable({
			...
			pk: id,
			savenochange: true,
			url: baseUrl('data_apbd/update_target_fisik_pergeseran/' + kode_sub_kegiatan + '/' + tahap + '/' + pergeseran_ke),
			success: function(c) {
...
function show_sub_kegiatan_apbd_instansi_gabungan_pergeseran()
	{
		var input_by = "<?php echo $input_by=="" ? "" : $input_by ?>";
		var fetch_method = "<?php echo $fetch_method; ?>";
		var tahun = "<?php echo $tahun; ?>";
		...
		$('#table-sub-kegiatan-instansi-gabungan_pergeseran').DataTable(
		{
	        processing	: true,
	        serverSide	: true,
	        bDestroy	: true,
	        responsive	: true,
	        ajax		: {
				          	url 	: baseUrl('data_apbd/sub_kegiatan_apbd_instansi_gabungan_pergeseran/' + input_by),
				            type 	: "POST",
				          	data 	: {
				          		tahun : tahun,
				          		tahap : kode_tahap,
...
function show_sub_kegiatan_apbd_instansi(id_table)
			  			...
			  			$.ajax(
						{
							...
							success : function(data)
							{
								if(data.status == true)
								{
									...
									else if (show=='instansi_teknis') {
										
									input_ski_teknis(data.kode_sub_kegiatan, kode_kegiatan, kode_program, tahap, 0, 0);
									$('#table-pilihan-sub-kegiatan-apbd').DataTable().ajax.reload(null, false);
	...
	$('#table-sub-kegiatan-instansi-gabungan_pergeseran').on('click', '#geser', function(){

		let sub_kegiatan 		= $(this).attr('sub_kegiatan');
		let id_ask 		= $(this).attr('id_anggaran');
		let pergeseran_selanjutnya 		= $(this).attr('pergeseran_selanjutnya');

		Swal.fire({
			  title: 'Warning',
			  html: 'Alihkan ke Pergeseran sub kegiatan <br> '+ sub_kegiatan +' <br> untuk pergeseran ke '+pergeseran_selanjutnya+'.?<br>' + id_ask,
			  icon: 'warning',
			  showCancelButton: true,
			  confirmButtonColor: '#3085d6',
			  cancelButtonColor: '#d33',
			  confirmButtonText: 'Alihkan pergeseran ke ' + pergeseran_selanjutnya,
			  cancelButtonText: 'Batal'
			}).then((result) => {
			  if (result.isConfirmed) {
			  			$.ajax(
						{
							url     : baseUrl('data_apbd/alihkan_pergeseran_sub_kegiatan_instansi/'),
							dataType: 'JSON',
							type    : 'POST',
							data    : { 
								id_ask : id_ask,
								pergeseran_selanjutnya : pergeseran_selanjutnya,
							...
							success : function(data)
							{
								if(data.status == true)
								{
									Swal.fire(
								      'Dialihkan!',
								      'Sub kegiatan Dialihkan ke Pergeseran ke '+ pergeseran_selanjutnya,
								      'success'
								    );

									$('#table-sub-kegiatan-instansi-gabungan_pergeseran').DataTable().ajax.reload(null, false);
								}
							},
							error : function(){
								Swal.fire('error','error','error');
	...
	// function input_pergeseran(nama_sub_kegiatan)
	// {
			  ...
			  if (result.isConfirmed) {
			  			$.ajax(
				        {
					        ...
					        error : function(){
								...
								Swal.fire(
						      'Error!',
						       'Copy Pagu, target, sumber dana pada APBD Awal sub kegiatan ' + nama_sub_kegiatan +' ke APBD Perubahan Gagal',
						      'error'
						    );
								$('#table-sub-kegiatan-instansi-gabungan').DataTable().ajax.reload(null, false);
									$('#table-sub-kegiatan-'+ id_table_kegiatan).DataTable().ajax.reload(null, false);
...
<script>
 function get_data_integrasi_sipd(){
		...
		$.ajax({
			url: baseUrl('integrasi_aplikasi/sipd'),
			type: 'POST',
			// dataType: 'JSON',
			data: {
				id_instansi : id_instansi,
