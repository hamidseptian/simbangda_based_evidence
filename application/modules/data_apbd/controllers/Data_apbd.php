...
class Data_apbd extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $id_kedudukan = $this->session->userdata('id_kedudukan');
         if ($id_kedudukan=='') {
            if ($tahap == 4) {
                ...
                $data['dropdown_option'] = [
                    // ['tipe'=>'link', 'caption'=>'Integrasi SIPD', 'fa'=>'metismenu-icon fas fa-list-ul', 'onclick'=>"data_apbd/integrasi_sipd", 'elemen_tambahan'=>'data-toggle="tooltip" title="Melihat semua data sub kegiatan yang ada pada SKPD"'],
                     ['tipe'=>'button', 'caption'=>'Pilih Data APBD', 'fa'=>'fa fa-cog fa-w-16 fa-spin', 'onclick'=>'setting_apbd()', 'elemen_tambahan'=>'data-toggle="tooltip" title="Memilih data program kegiatan dan sub kegiatan SKPD"'], 
        ...
        $data['id_instansi']                = $id_instansi;
          $bu       = $this->db->query("SELECT kode_bidang_urusan, nama_bidang_urusan from master_bidang_urusan")->result_array();
            $kumpul_bu = [];
            foreach ($bu as $k => $v) {
                $kumpul_bu[$v['kode_bidang_urusan']] = $v['nama_bidang_urusan'];
            }
       ...
       public function target_data_apbd()
    {
        $breadcrumbs     = $this->breadcrumbs;
        $master_paket   = $this->master_paket_model;
        $data_apbd_model   = $this->data_apbd_model;
        ...
        $breadcrumbs->add('Home', base_url());
        $breadcrumbs->add('Target Data APBD ', base_url($this->router->fetch_class()));
        ...
        $data['icon']                       = "metismenu-icon fa fa-list-ul";
        $data['description']                = "Menampilkan Data Target APBD";
        $data['breadcrumbs']                = '';//$breadcrumbs->render();
        ...
        $page                               = 'data_apbd/data_apbd/target_data_apbd';
        $data['link']                       = $this->router->fetch_method();
        $data['fetch_method']                       = $this->router->fetch_method();
        $data['menu']                       = $this->load->view('layout/menu', $data, true);
        $data['extra_css']                  = $this->load->view('data_apbd/data_apbd/css', $data, true);
    ...
    public function pergeseran()
    {
        ...
        $data['tot_subkeg']                 = $data_apbd_model->total()['subkeg'];
        $data['tot_bu']                     = $data_apbd_model->total()['bu'];
        $page                               = 'data_apbd/data_apbd/pergeseran';
        $data['link']                       = $this->router->fetch_method();
        $data['fetch_method']                =  $this->router->fetch_method();
        $data['menu']                       = $this->load->view('layout/menu', $data, true);
        $data['extra_css']                  = $this->load->view('data_apbd/data_apbd/css', $data, true);
    ...
    public function sub_kegiatan($input_by)
    {
        ...
        $tahun = tahun_anggaran();
         ...
         $program = $this->db->query("SELECT kode_program, nama_program from master_program where status = 1")->result_array();
            $kumpul_program = [];
        ...
        $data['kumpul_program']                = $kumpul_program;
        $data['kode_tahap']                = $tahap;
        $data['tahun']                = $tahun;
        $data['title']                      = "Data APBD";
        $data['icon']                       = "metismenu-icon fa fa-list-ul";
...
public function sync_eplanning()
        ...
        if(!$this->input->is_ajax_request())
        {
            ...
            foreach ($list as $lists)
            {
                ...
                if ($tahap==4) {
                    ...
                    $pagu = $q_pagu['pagu'] =='333' ? 0 : $q_pagu['pagu'];
    ...
    public function update_target_fisik_pergeseran($kode_rekening_sub_kegiatan, $tahap, $pergeseran_ke)
    {
        $id_target_apbd = sbe_crypt($this->input->post('pk'), 'D');
        $target_fisik   = $this->input->post('value');
        $tahun = tahun_anggaran();
    ...
    public function sub_kegiatan_apbd_instansi_gabungan($input_by)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $order          = ['nama_sub_kegiatan' => 'ASC'];
            $list           = $this->datatables_model->get_datatables('sub_kegiatan_instansi', $column_order, $column_search, $order, $where);
            $data           = [];
            $no             = $_POST['start'];
            ...
            foreach ($list as $lists) {
                $krsk = $lists->kode_sub_kegiatan;
                $pecah = explode('.', $lists->kode_sub_kegiatan);
                $kode_sub_kegiatan = $pecah[0].'.'.$pecah[1].'.'.$pecah[2].'.'.$pecah[3].'.'.$pecah[4].'.'.$pecah[5];
                $kategori = $lists->kategori;
                ...
                $no++;
                $kode_rekening_program = $lists->kode_program;
                ...
                $pergeseran  = $this->db->query("SELECT id_anggaran_sub_kegiatan, (bo_bp + bo_bbj + bo_bs + bo_bh + bm_bmt + bm_bmpm + bm_bmgb + bm_bmjji + bm_bmatl + btt + bt_bbh + bt_bbk ) as pagu,
                    lakukan_pergeseran, pergeseran_ke, pagu_berubah, berubah_pada_pergeseran_ke from anggaran_sub_kegiatan where kode_sub_kegiatan = '$krsk' and tahun = '$tahun' and kode_tahap = $tahap_ski and id_instansi='$id_instansi'")->row_array(); 
                if ($tahap_aktif==4) {
                	$caption_tahapan = $caption_status_tahapan[$lists->kode_tahap];
                	$caption_status_sub_kegiatan = $status_sub_kegiatan[$lists->status];
                    $pagu_saat_ini = $pergeseran['pagu'];

                           $onclick = "hapus_sub_kegiatan_instansi('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."', '".$lists->tahun."','gabungan')";
                        $onclick2 = "input_anggaran('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."','".tahun_anggaran()."','".$lists->kode_bidang_urusan."','gabungan','".$pergeseran['lakukan_pergeseran']."','".$pergeseran['pagu_berubah']."','".$pergeseran_ke_saat_ini."','".$pergeseran['berubah_pada_pergeseran_ke']."')";
                        $onclick3 = "get_target('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."','".tahun_anggaran()."','".$lists->kode_bidang_urusan."','".$pagu_saat_ini."')";
                        $onclick4 = "get_sumber_dana('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."','".tahun_anggaran()."','".$lists->kode_bidang_urusan."','".$pagu_saat_ini."')";
                    ...
                    if ($pergeseran['pagu_berubah']=='') {

                        if ($pergeseran['lakukan_pergeseran']=='') {
                            $nama_tahapan = "APBD AWAL";
                            # code...
                        }else{
                            $pergeseran_ke_saat_ini = $pergeseran['pergeseran_ke'];
                            $nama_tahapan = "APBD PERGESERAN <br>Pergeseran Ke ". $pergeseran_ke_saat_ini;

                        }
                        $pergeseran_ke_baru = 1;
                        $pagu_saat_ini = $pergeseran['pagu'];
                        ...
                        $warna_pagu = 'color:blue';
                        $pergeseran_ke_baru = $pergeseran['pergeseran_ke']+1;
                        
                        $pergeseran_ke_saat_ini = $pergeseran['pergeseran_ke'];
                        $q_pagu_pergeseran = $this->db->query("SELECT (bo_bp + bo_bbj + bo_bs + bo_bh + bm_bmt + bm_bmpm + bm_bmgb + bm_bmjji + bm_bmatl + btt + bt_bbh + bt_bbk ) as pagu_pergeseran from anggaran_sub_kegiatan_pergeseran where kode_sub_kegiatan = '$krsk' and tahun = '$tahun' and kode_tahap = 2 and pergeseran_ke='$pergeseran_ke_saat_ini'")->row_array();
                        $pagu_saat_ini = $q_pagu_pergeseran['pagu_pergeseran'];
                        $nama_tahapan = "APBD PERGESERAN <br>Pergeseran Ke ". $pergeseran_ke_saat_ini;



                        $onclick = "hapus_sub_kegiatan_instansi('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."', '".$lists->tahun."','gabungan')";
                        $onclick2 = "input_anggaran_pergeseran('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."','".tahun_anggaran()."','".$lists->kode_bidang_urusan."','".$pergeseran_ke_saat_ini."','gabungan')";
                        $onclick3 = "get_target_pergeseran('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."','".tahun_anggaran()."','".$lists->kode_bidang_urusan."','".$pagu_saat_ini."','".$pergeseran_ke_saat_ini."')";
                        $onclick4 = "Swal.fire('dev','Dalam Pengmbangan','error')";//"get_sumber_dana('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".$lists->kode_tahap."','".tahun_anggaran()."','".$lists->kode_bidang_urusan."','".$pagu_saat_ini."')";
                ...
                $row[]  = '<b>'.$lists->kode_program.'</b><br>'. $kumpul_program[$lists->kode_program];
                $row[]  = '<b>'.$lists->kode_kegiatan.'</b><br>'. $kumpul_kegiatan[$lists->kode_kegiatan];
                $row[]  = $nama_sub_kegiatan ; 
                $row[]  = '<b>'.$lists->kategori;
                 $pagu   = $pagu_saat_ini =='' ? 0 : $pagu_saat_ini;
                $row[]  =  '<span style="float:right; '.$warna_pagu.'">'.number_format($pagu,0,'','.').'</span>';
                // $row[]  = $lists->input_by;
               ...
               $onclick_copy = "copy_data_apbd_sub_kegiatan('".$lists->kode_sub_kegiatan."','".$lists->kode_kegiatan."', '".$lists->kode_program."','".tahapan_apbd()."','".$lists->kode_bidang_urusan."','".$lists->kode_sub_kegiatan."')";
            ...
            $output = [
                "draw"              => $_POST['draw'],
                "recordsTotal"      => $this->datatables_model->count_all('sub_kegiatan_instansi', $where),
                "recordsFiltered"   => $this->datatables_model->count_filtered('sub_kegiatan_instansi', $column_order, $column_search, $order, $where),
                "data"              => $data,
            ];
    ...
    public function sub_kegiatan_apbd_instansi_gabungan_pergeseran($input_by)
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {            
            ...
            foreach ($list as $lists) {
                ...
                // $row[]  = $caption_tahapan;
                 ...
                 $pergeseran  = $this->db->query("SELECT id_anggaran_sub_kegiatan, (bo_bp + bo_bbj + bo_bs + bo_bh + bm_bmt + bm_bmpm + bm_bmgb + bm_bmjji + bm_bmatl + btt + bt_bbh + bt_bbk ) as pagu,
                    lakukan_pergeseran, pergeseran_ke, pagu_berubah, berubah_pada_pergeseran_ke from anggaran_sub_kegiatan where kode_sub_kegiatan = '$krsk' and tahun = '$tahun' and kode_tahap = 2 and id_instansi = '$id_instansi'")->row_array();
                ...
                if($pergeseran['lakukan_pergeseran']==''){
                    ...
                    if ($pergeseran['pagu_berubah']=='') {
                        $pergeseran_ke_baru = 1;
                        $pagu_saat_ini = $pergeseran['pagu'];
                        $nama_tahapan = "APBD PERGESERANsdsddsds <br>Pergeseran Ke ". $pergeseran_ke_saat_ini;
                    }else{
                        $pergeseran_ke_baru = ($pergeseran['pergeseran_ke'] == '' ? 0 : $pergeseran['pergeseran_ke'] )+1;
                        $pergeseran_ke_saat_ini = $pergeseran['berubah_pada_pergeseran_ke'];
                        $q_pagu_pergeseran = $this->db->query("SELECT (bo_bp + bo_bbj + bo_bs + bo_bh + bm_bmt + bm_bmpm + bm_bmgb + bm_bmjji + bm_bmatl + btt + bt_bbh + bt_bbk ) as pagu_pergeseran from anggaran_sub_kegiatan_pergeseran where kode_sub_kegiatan = '$krsk' and tahun = '$tahun' and kode_tahap = 2 and pergeseran_ke='$pergeseran_ke_saat_ini'")->row_array();
                        $pagu_saat_ini = $q_pagu_pergeseran['pagu_pergeseran'];
                        $nama_tahapan = "APBD PERGESERAN <br>Pergeseran Ke ". $pergeseran_ke_saat_ini;
                ...
                $row[]  = '<b>'.$lists->kategori;
                 $pagu   =  $pagu_saat_ini =='' ? 0 : $pagu_saat_ini;
                $row[]  = $nama_tahapan ;//$lists->input_by;
                $row[]  =  '<span style="float:right">'.number_format($pagu,0,'','.').'</span>';
                    ...
                    if (jadwal_input_data_dasar()['aktif']==0) {
                        ...
                        if ($id_kedudukan=='') {
                            $tombol_pergeseran_baru = '<button class="btn btn-outline-info btn-xs"  id="geser" title="Input target sub   kegiatan '.$lists->nama_sub_kegiatan.'" sub_kegiatan="'.$nama_sub_kegiatan.'" id_anggaran="'.$pergeseran['id_anggaran_sub_kegiatan'].'" pergeseran_selanjutnya="'.$pergeseran_ke_baru.'"><i class="fas fa-angle-double-right"></i></button>';
                            $tombol_pergeseran_sebelumnya = '<button class="btn btn-outline-danger btn-xs" onclick="Swal.fire(`Pesan` , `Jika tombol ini di klik akan enampilkan pilihan pergeseran dan apbd awal`,`warning`)" title="Input target sub   kegiatan '.$lists->nama_sub_kegiatan.'" sub_kegiatan="'.$nama_sub_kegiatan.'" id_anggaran="'.$pergeseran['id_anggaran_sub_kegiatan'].'" pergeseran_selanjutnya="'.$pergeseran_ke_baru.'"><i class="fas fa-angle-double-right"></i></button> ';
                              $row[]  = $tombol_pergeseran_baru ;//. $tombol_pergeseran_sebelumnya;
                        }else{

                            if (in_array($krsk, $kumpul_usk_pptk)) {
                                    # code...
                                 // $row[]  = '<button class="btn btn-outline-danger btn-xs"  title="Input target sub   kegiatan '.$lists->nama_sub_kegiatan.'"  onclick="input_pergeseran('.".".$nama_sub_kegiatan.".".')"><i class="fas fa-money-bill"></i></button> ';
                            }else{
                                $row[]     = ' <div class="btn-group">
                        <button class="btn btn-outline-danger btn-sm" onclick="'.$warning_bukan_kegiatan_anda.'">Terkunci</button>
                        </div> ';
    ...
    public function pengalihan_apbd_sub_kegiatan_apbd_instansi_gabungan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($tahap==4) {
                ...
                $where          = 'id_instansi='.id_instansi().' and tahun='.$tahun.' and kode_tahap in (2,3) and kode_tahap_pengecekan_pengalihan_data is null';
            }else {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($target->num_rows() > 0) {
                foreach ($target->result() as $key => $value) {
                    ...
                    $output['data'][$key]['input_by'] = $value->input_by == '' ? 'Manual Input' : $value->input_by;
                }

                $output['status']  = true;
                $alert_kunci_input = "Swal.fire('Terkunci','Mohon maaf, <br>Anda tidak dapat menguah target<br>".jadwal_input_data_dasar()['pesan']."','error')";
                $output['kunci']  = jadwal_input_data_dasar()['aktif'];
                $output['alert_kunci']  = $alert_kunci_input;
 ...
 public function get_target_pergeseran()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
            ...
            $target                 = $this->db->get_where('target_apbd_pergeseran', $where);
            $subkeg                 = $this->db->query("SELECT nama_sub_kegiatan, kategori, jenis_sub_kegiatan, keterangan from v_sub_kegiatan_apbd where kode_rekening_sub_kegiatan='$kode_rekening_sub_kegiatan' and kode_tahap='$tahap' and id_instansi='$id_instansi' and tahun = '$tahun'")->row();
            $nama_sub_kegiatan = $subkeg->kategori =='Sub Kegiatan SKPD' ? $subkeg->nama_sub_kegiatan : $subkeg->nama_sub_kegiatan.'<br>'.$subkeg->jenis_sub_kegiatan.' - '.$subkeg->keterangan;
            ...
            $output['nama_sub_kegiatan']     = $nama_sub_kegiatan;
            $output['kode_sub_kegiatan']     = $kode_sub_kegiatan;
            $output['nama_tahapan']  = "APBD PERGESERAN <br>Pergeseran Ke ".$pergeseran_ke;
            if ($target->num_rows() > 0) {
                foreach ($target->result() as $key => $value) {
                    $output['data'][$key]['id']         = sbe_crypt($value->id_target_apbd, 'E');
                    $output['data'][$key]['bulan']      = $value->bulan;
                ...
                for ($i=0; $i < 12; $i++) { 
                    ...
                    $cek = $this->db->get_where('target_apbd_pergeseran', $wherenull);
                    if ($cek->num_rows()==0) {
                        $insert = [
                            'id_instansi'=>id_instansi(),
                            'kode_bidang_urusan'=>$kode_bidang_urusan,
                        ...
                        $this->db->insert('target_apbd_pergeseran', $insert);
                    }
                }
            }
 ...
 public function copy_data_apbd_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
           ...
           $usk = $this->db->get_where('users_sub_kegiatan',$where_usk_lama);
            ...
            $kumpul_realisasi_keuangan_baru = [];
            foreach ($realisasi_keuangan->result_array() as $k => $v) {
                 $data_realisasi_keuangan = [
                    'kode_bidang_urusan'=>$v['kode_bidang_urusan'],
                    ...
                    'created_on'=>timestamp(),
                    'created_by'=>$v['created_by'],
                    'input_by'=>"Copy APBD Awal",
                 
                   ];
                   array_push($kumpul_realisasi_keuangan_baru, $data_realisasi_keuangan);
            }
            ...
            // $output['cek'] = $anggaran_awal->num_rows();
               ...
               $data_usk_baru = [
                'id_instansi'=>$usk_apbd_awal['id_instansi'],
                'id_user'=>$usk_apbd_awal['id_user'],
                'kode_rekening_sub_kegiatan'=>$usk_apbd_awal['kode_rekening_sub_kegiatan'],
                'kode_rekening_kegiatan'=>$usk_apbd_awal['kode_rekening_kegiatan'],
                'kode_rekening_program'=>$usk_apbd_awal['kode_rekening_program'],
                'kode_bidang_urusan'=>$usk_apbd_awal['kode_bidang_urusan'],
                'kode_tahap'=>tahapan_apbd(),
                'tahun_anggaran'=>$usk_apbd_awal['tahun_anggaran'],
            ...
            if ($anggaran_perubahan->num_rows()==0) {
               ...
               if ($usk->num_rows()>0) {
               $this->db->insert('users_sub_kegiatan', $data_usk_baru);
               }
               if ($realisasi_keuangan->num_rows()>0) {
               $this->db->insert_batch('realisasi_keuangan', $kumpul_realisasi_keuangan_baru);
               }
            }
 ...
 public function copy_sub_kegiatan_data_apbd()
    ...
    public function get_anggaran_sub_kegiatan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $identitas_ski        = $this->db->query("SELECT ski.nama_sub_kegiatan, ski.kode_sub_kegiatan, ski.kategori, ski.keterangan, ski.jenis_sub_kegiatan from sub_kegiatan_instansi ski 
            ...
            if ($pagu_sub_kegiatan->num_rows() > 0) {
                $value = $pagu_sub_kegiatan->row();
                       $pagu_total = $value->bo_bp + $value->bo_bbj + $value->bo_bs + $value->bo_bh + $value->bm_bmt + $value->bm_bmpm + $value->bm_bmgb + $value->bm_bmjji + $value->bm_bmatl + $value->btt + $value->bt_bbh + $value->bt_bbk;
                if ($tahap==2) {
                    if ($value->lakukan_pergeseran=='') {
                        // $output['data']['pagu_aktif']                  = "APBD AWAL";
                        $output['data']['tahapan']                  = "APBD AWAL";

                    }else{
                        if ($value->pagu_berubah=='Ya') {
                            $output['data']['tahapan']                  = "APBD PERGESERAN<br>Pergeseran Ke-".$value->pergeseran_ke;
                            // $output['data']['pagu_aktif']                  = "APBD PERGESERAN<br>Pergeseran Ke-".$value->pergeseran_ke;
                            # code...
                        }else{
                            $output['data']['tahapan']                  = "APBD PERGESERAN<br>Pergeseran Ke-".$value->pergeseran_ke;
                            // $output['data']['pagu_aktif']                  = "APBD AWAL<br>Belum ada perubahan pagu";
    ...
    public function get_anggaran_sub_kegiatan_pergeseran()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
            ...
            $pergeseran_ke = $this->input->post('pergeseran_ke');
            $id_instansi = id_instansi();
            $where = ['kode_sub_kegiatan' => $kode_rekening_sub_kegiatan, 'kode_kegiatan'=>$kode_kegiatan, 'kode_program'=>$kode_program, 'kode_tahap'=>$tahap,'tahun'=>$tahun,  'pergeseran_ke'=>$pergeseran_ke,   'id_instansi' => id_instansi()];

            $pagu_sub_kegiatan    = $this->db->get_where('anggaran_sub_kegiatan_pergeseran', $where);
    ...
    public function simpan_pengalihan_sub_kegiatan_instansi_apbd_awal_ke_perubahan()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $usk = $this->db->get_where('sub_kegiatan_instansi', ['id_sub_kegiatan_instansi'=>$id_sub_kegiatan_instansi])->row_array();

            $data_ski_baru = [
                'kode_sub_kegiatan'=>$ski['kode_sub_kegiatan'],
                'nama_sub_kegiatan'=>$ski['nama_sub_kegiatan'],
                'nama_sub_kegiatan'=>$ski['nama_sub_kegiatan'],
                ...
                'id_instansi_pembantu_teknis'=>$ski['id_instansi_pembantu_teknis'],
            ...
            $where_usk_lama = [
                'id_instansi'=> $ski['id_instansi'],
                'kode_rekening_sub_kegiatan'=> $ski['kode_sub_kegiatan'],
                'kode_rekening_kegiatan'=> $ski['kode_kegiatan'],
                'kode_rekening_program'=> $ski['kode_program'],
                'kode_bidang_urusan'=> $ski['kode_bidang_urusan'],
                'tahun_anggaran '=> $ski['tahun']
            ];
              
            $this->db->update('users_sub_kegiatan', ['status'=>0],$where_usk_lama);
    ...
    public function alihkan_pergeseran_sub_kegiatan_instansi()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
            ...
            $pergeseran_selanjutnya = $this->input->post('pergeseran_selanjutnya');
            $q = $this->db->query("UPDATE anggaran_sub_kegiatan set pergeseran_ke='$pergeseran_selanjutnya', lakukan_pergeseran='1' where id_anggaran_sub_kegiatan='$id_ask'");
    ...
    public function save_ski_teknis()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if ($validation->run($this)) {
                ...
                $q_msk              = $this->db->query("SELECT * from master_sub_kegiatan where kode_sub_kegiatan='$kode_sub_kegiatan'")->row_array();
                $kode_bidang_urusan = $q_msk['kode_bidang_urusan'];
                $tahap = $post['tahap'];
                ...
                $kode_ski_teknis    = $kode_sub_kegiatan.'.'.$kode;
                 ...
                 $data = [
                            'nama_sub_kegiatan'=>$q_msk['nama_sub_kegiatan'],
...
$kode_rekening_sub_kegiatan = $this->input->post('kode_sub_kegiatan');
    ...
    public function save_anggaran_sub_kegiatan_pergeseran()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        } else {
            ...
            $validation->set_error_delimiters('<p class="text-danger">', '</p>');
                ...
                $tahun = $this->input->post('tahun');
                $pergeseran_ke = $this->input->post('pergeseran_ke');
                $pagu_berubah = $this->input->post('pagu_berubah');



                $where_awal = ['kode_sub_kegiatan' => $kode_rekening_sub_kegiatan, 'kode_kegiatan'=>$kode_kegiatan, 'kode_program'=>$kode_program, 'kode_tahap'=>$tahap,  'tahun'=>$tahun,  'id_instansi' => id_instansi()];
                $pagu_sub_kegiatan_awal    = $this->db->get_where('anggaran_sub_kegiatan', $where_awal)->row_array();
                if ($pagu_sub_kegiatan_awal['pagu_berubah']=='') {
                    $update_ask_awal = ['pagu_berubah'=>'Ya', 'berubah_pada_pergeseran_ke'=>$pergeseran_ke];
                    $this->db->update('anggaran_sub_kegiatan', $update_ask_awal, $where_awal);
                }




                $where = ['kode_sub_kegiatan' => $kode_rekening_sub_kegiatan, 'kode_kegiatan'=>$kode_kegiatan, 'kode_program'=>$kode_program, 'kode_tahap'=>$tahap,  'pergeseran_ke'=>$pergeseran_ke,   'id_instansi' => id_instansi()];
            ...
            if ($validation->run($this)) {
                ...
                // $pagu_sub_kegiatan    = $this->db->get_where('anggaran_sub_kegiatan', $where);
                if ($pagu_sub_kegiatan->num_rows()>0) {
                    $id_paket_pekerjaan = $data_apbd->saveedit_anggaran_sub_kegiatan_pergeseran($where);
                    
                }else{
                    $id_paket_pekerjaan = $data_apbd->save_anggaran_sub_kegiatan_pergeseran();
                }
 ...
 public function hapus_all_apbd_instansi()
    ...
    public function tambah_sub_kegiatan_instansi()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $kode_sub_kegiatan = $this->input->post('kode_sub_kegiatan');
            $q_sub_kegiatan = $this->db->query("SELECT nama_sub_kegiatan from master_sub_kegiatan where kode_sub_kegiatan='$kode_sub_kegiatan'")->row_array();
            $kode_kegiatan = $this->input->post('kode_kegiatan');
            $kode_program = $this->input->post('kode_program');
            $kode_bidang_urusan = $this->input->post('kode_bidang_urusan');
            $tahun = tahun_anggaran();
