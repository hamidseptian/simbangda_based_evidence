...
class Synchronize extends MY_Controller
{
    ...
    public function realisasi_fisik($id_instansi)
    {
        ...
        <table border=1 style='border-collapse:collapse'>
            ...
            <td rowspan='3'>Jumlah Paket</td>
            <td colspan='".(date('n') *3)."'>Bulan</td>
            </tr>
            <tr>
        ";
        for ($a = 1; $a <= date('n'); $a++) {
        ...
        foreach ($kegiatan as $key => $value) {
            ...
            $jenis_swakelola    = $this->target_realisasi_model->total_paket_perjenis($value->kode_rekening, 'SWAKELOLA')->num_rows();
            $jenis_penyedia    = $this->target_realisasi_model->total_paket_perjenis($value->kode_rekening, 'PENYEDIA')->num_rows();
            $jenis_rutin    = $this->target_realisasi_model->total_paket_perjenis($value->kode_rekening, 'RUTIN')->num_rows();
             echo "
             <tr>
            <td>".$no++."</td>
        ...
        ";
            ...
            for ($i = 1; $i <= date('n'); $i++) {
                ...
                $rutin          = $jenis_rutin >0 ? ($jenis_rutin   * $this->rutin($i)) : 0 ;
                echo "<td>".$swakelola."</td>";
                echo "<td>".$penyedia."</td>";
                echo "<td>".$rutin."</td>";
                ...
                $total_fisik_bulan[$i][] = $total_fisik_perkegiatan;
            }
        }
           echo "
           </tr>
            ...
            <td colspan='3'>Total (Rata-rata )</td>";
                foreach ($total_fisik_bulan   as $key => $value) {
                    echo " <td colspan='3'>".array_sum(explode(',', implode(',', $total_fisik_bulan[$key]))) / count($kegiatan)."</td>";
                }
            echo "
            </tr>
        </table>";

        echo json_encode(   $total_fisik_bulan );    
