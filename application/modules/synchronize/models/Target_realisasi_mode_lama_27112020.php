...
class Target_realisasi_model extends CI_Model
{
	...
	{
		return $this->db->query("SELECT SUM(pagu) AS pagu FROM v_program_apbd WHERE id_instansi = '{$id_instansi}'")->row()->pagu;
	}
	...
	public function get_target_keuangan($id_instansi)
	{
		$query  = $this->db->query("SELECT
									...
									FROM
										target_apbd ta
										LEFT JOIN master_instansi mi ON ta.id_instansi = mi.id_instansi
										LEFT JOIN kegiatan_apbd ka ON ta.kode_rekening = ka.kode_rekening
										WHERE 
