...
<script>
	...
	$('#table-kpa').on('click', '#list-paket', function()
	{
		...
		let anggaran_terpakai 		= $(this).attr('anggaran_terpakai');
		let tr_detail_paket	= '';

		tr_detail_paket  = '<tr colspan="6" class="card-body formulira" id="'+ id_detail +'" rowspan="1">';
		tr_detail_paket +=     '<td rowspan="1" colspan="6">';
		tr_detail_paket += 		'<button class="btn btn-info btn-sm mb-2" onclick="addMasterPaket('+"'"+ id_pptk +"','"+ kode_rekening +"','"+ kode_urusan +"','"+ pagu +"','"+ anggaran_terpakai +"'"+')">Tambah Paket</button>';
	...
	function edit_paket(id_paket_pekerjaan)
	{
		status_paket = 'update';
                ...
                if(data.status == true)
                {
                	$.each(data.data, function(k, v){
                		$('#list-kegiatan-pptk-'+ id_user).append('<tr>' +
                								'<td><button id="list-paket" status="collapse" id-pptk="'+ v.id_pptk +'" kode-rekening="'+ v.rekening +'" kode-urusan="'+ v.kode_urusan +'"  pagu="'+ v.pagu_kegiatan +'"  anggaran_terpakai="'+ v.anggaran_terpakai +'" class="btn btn-info btn-xs">+</button></td>' +
				                                '<td>'+ v.rekening +'</td>' +
				                                '<td>'+ v.kegiatan +'</td>' +
				                                '<td>'+ v.pagu +'</td>' +
				                                '<td>'+ v.jml_paket +' Paket</td>' +
	...
	function addMasterPaket(id_pptk, kode_rekening, kode_urusan, pagu, anggaran_terpakai)
	{
		status_paket = 'save';
		var anggaran_tersedia = pagu - anggaran_terpakai;
	    $('#modalMasterPaket').modal('show')
					  		  .find('.modal-title').html('Tambah Paket<br>Anggaran Tersedia : ' + convert_ke_rupiah(anggaran_tersedia));
		$('.form-control').removeClass('is-valid')
						  .removeClass('is-invalid');
        $('.text-danger').remove();
	...
	function list_kab_kota(id_paket='')
	{
		$.ajax(
        {
	...
	function lokasi(id_paket_pekerjaan)
	{
	...
	function tambah_lokasi()
	{
		...
		if(id_kab_kota)
		{
			$.ajax(
			{
				...
				dataType: 'JSON',
				type    : 'POST',
				data    : { id_paket : id_paket, id_kab_kota : id_kab_kota },
				success : function(data)
				{
					if(data.status == true)
					{
	...
	function volume(id_paket_pekerjaan)
	{
	...
	function tambah_vol()
	{
		...
		$.ajax(
		{
			...
			dataType: 'JSON',
			type    : 'POST',
			data    : { id_paket : id_paket, nama_pelaksanaan : nama_pel },
			success : function(data)
			{
				if(data.status == true)
				{
	...
	function delete_paket(id_paket_pekerjaan)
	{
		$.ajax(
		{
  ...
  //   }
	...
	function delete_vol(id_vol_pelaksanaan_pekerjaan, id_paket_pekerjaan , pelaksanaan_ke)
	{
