...
<script>
	...
	$('#table-kpa').on('click', '#list-paket', function()
	{
		...
		tr_detail_paket += 		'<button class="btn btn-info btn-sm mb-2" onclick="addMasterPaket('+"'"+ id_pptk +"','"+ kode_sub_kegiatan +"','"+ kode_kegiatan +"','"+kode_program+"','"+ kode_bidang_urusan+"')" +'">Tambah Paket</button>';
		...
		$.ajax(
        {
            ...
            success : function(data)
            {
                if(data.status == true)
                {
                	...
                	var pagu_tersedia = anggaran_tersedia(data.data.kode_rekening_sub_kegiatan, data.data.kode_rekening_kegiatan , data.data.kode_rekening_program, data.data.kode_bidang_urusan );
...
$('#modalMasterPaket').find('.modal-title').html('Tambah Paket<br>Anggaran Tersedia : ' + convert_ke_rupiah(pagu_dibolehkan));
	...
	function addMasterPaket(id_pptk, kode_sub_kegiatan, kode_kegiatan, kode_program, kode_bidang_urusan)
	{
		var kode_rekening = '';
		
		$('#formMasterPaket')[0].reset();
		
		var pagu_tersedia = anggaran_tersedia(kode_sub_kegiatan, kode_kegiatan, kode_program, kode_bidang_urusan);
	...
	function anggaran_tersedia(kode_sub_kegiatan, kode_kegiatan, kode_program, kode_bidang_urusan)
	{
