...
class Integrasi_dashboard_pembangunan_model extends CI_Model
{
	...
	public function laporan_opd($tahun, $kode_tahap, $bulan, $id_instansi='' , $urutan_ranking='', $ranking_berdasarkan='')
	{	
		if ($id_instansi=='') {
			if ($urutan_ranking=='') {
				$q = $this->db->query("SELECT mi.nama_instansi, mi.singkatan_nama_instansi, mi.id_parent, mi.integrasi_sipedal_id_instansi,
				g.pagu_total, g.rp_target_keuangan_akumulasi, g.rp_target_keuangan_bulanan, g.rp_realisasi_keuangan_akumulasi, g.rp_realisasi_keuangan_bulanan,
				...
				// $ranking_berdasarkan = "";
				if ($ranking_berdasarkan == 'deviasi_keuangan') {
					$kolom_dirankingkan = "(realisasi_keuangan_akumulasi - target_keuangan_akumulasi)";
				$sort = $urutan_ranking == 'tertinggi' ? "DESC" : "ASC";
				$order_by = "order by $kolom_dirankingkan $sort ";
					# code...
				}else{
					$kolom_dirankingkan = $ranking_berdasarkan == 'keuangan' ? "realisasi_keuangan_akumulasi" : "realisasi_fisik_akumulasi";
				$sort = $urutan_ranking == 'tertinggi' ? "DESC" : "ASC";
				$order_by = "order by $kolom_dirankingkan $sort ";
			...
			$q = $this->db->query("SELECT mi.nama_instansi, mi.singkatan_nama_instansi, g.pagu_total, g.rp_target_keuangan_akumulasi, g.rp_target_keuangan_bulanan,  g.rp_realisasi_keuangan_akumulasi, g.rp_realisasi_keuangan_bulanan,
				 g.target_fisik_akumulasi, g.target_fisik_bulanan, g.realisasi_fisik_akumulasi, g.realisasi_fisik_bulanan , 
				 g.id_instansi, g.tahun, g.kode_tahap, g.bulan
				from grafik g  
				left join master_instansi mi on g.id_instansi = mi.id_instansi
	...
	public function opd_belum_ada_data($tahun, $tahap, $bulan)
	{	
	
		$q = $this->db->query("SELECT nama_instansi, id_instansi, integrasi_sipedal_id_instansi, total_anggaran_skpd_awal(id_instansi , $tahun) as pagu_total from master_instansi where 
			kategori ='OPD' and is_active='1' and 
			id_instansi not in (select id_instansi from grafik where tahun='$tahun' and kode_tahap='$tahap' and bulan='$bulan')");
		return $q;
	}
