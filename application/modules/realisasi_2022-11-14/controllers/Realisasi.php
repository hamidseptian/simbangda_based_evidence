 ...
 protected function jumlah_sub_kegiatan($id_user)
    ...
    public function dt_paket()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $tahun  = tahun_anggaran();
            $where          = ['id_instansi' => id_instansi(), 'tahun'=>$tahun, 'kode_rekening_sub_kegiatan' => $kode_rekening];
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                $nama_sub_kegiatan = $lists->nama_sub_kegiatan . str_replace('<br>', ' \n ', $keterangan) ;
                 $onclick_copy = "copy_realisasi_k_sub_kegiatan('".$lists->kode_rekening_sub_kegiatan."','".$lists->kode_rekening_kegiatan."', '".$lists->kode_rekening_program."','".tahapan_apbd()."','".$lists->kode_bidang_urusan."','".$nama_sub_kegiatan."')";
               ...
               $tombol_hapus = '<button class="btn btn-danger btn-xs"  title="Hapus realisasi keuangan sub kegiatan '.$lists->nama_sub_kegiatan.'"  onclick="'.$onclick_hapus.'"><i class="fas fa-trash"></i></button> ';
                ...
                $cek_count_data_realisasi = $this->db->query("SELECT id_realisasi_keuangan from realisasi_keuangan where id_instansi='$id_instansi' and kode_tahap='$tahap' and kode_sub_kegiatan='$krsk'")->num_rows();
                if (tahapan_apbd()!=2 && $total_realisasi==0 && $cek_count_data_realisasi==0 && $lists->pagu >0) {
                $tombol = $tombolcopy;
                }else{
