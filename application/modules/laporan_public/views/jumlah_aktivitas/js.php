...
<script>
	...
	$('#laporan_akhir').click(function(){
			$('#tampil_pdf').attr('src', baseUrl('laporan/laporan_realisasi_fisik_keuangan'));
	...
	$('#sync_all').click(function(){
		$('#sync_all').text('Loading...')
			.attr('disabled', true);
		$.ajax({
			url: baseUrl('laporan/sync_all'),
			// type: 'POST',
			// dataType: 'JSON',
			// data: {
			...
			success: function(data) {
				// if (data.status == true) {
					$('#sync_all').attr('class', 'btn btn-success');
					$('#sync_all').html('Syncrhronize All');
					$('#sync_all').attr('disabled', false);
					// window.location.href = "<?php echo base_url(); ?>laporan/jumlah_aktivitas";
				// }
			}
		});
