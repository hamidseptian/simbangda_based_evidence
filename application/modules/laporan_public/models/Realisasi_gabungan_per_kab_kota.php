...
class Realisasi_gabungan_per_kab_kota extends CI_Model
{
	...
	public function pagu_kota($id_kota, $tahap, $tahun)
	{
		...
		from v_instansi_kab_kota where id_kota='$id_kota' and kode_tahap='$tahap' and tahun = '$tahun'";
    ...
    public function cek_realisasi_dipilih($tahap, $tahun, $id_kota)
    {
        $bulan  = date('n');
        $query  = $this->db->query("SELECT
                                    ...
                                    WHERE
                                    id_kota = '$id_kota' and kode_tahap='$tahap' and tahun='$tahun'
                                        ");
        return $query;
    }

     public function total_realisasi_perjenis($bulan_aktif, $tahap, $tahun, $id_kota, $jenis)
    {
        if ($jenis=='realisasikan_bo') {
             $query  = $this->db->query("SELECT
                                    ...
                                    WHERE
                                    id_kota = '$id_kota' and kode_tahap='$tahap' and tahun='$tahun' and bulan <= $bulan_aktif
                                        ");
        }
            ...
            $query  = $this->db->query("SELECT
                                         sum( rf_total ) as realisasi_rt_total
                                    FROM
                                        realisasi_fisik_keuangan_kab_kota 
                                    WHERE
                                    id_kota = '$id_kota' and kode_tahap='$tahap' and tahun='$tahun' and bulan <= $bulan_aktif
    ...
    public function banyak_realisasikan($tahun, $tahap, $id_kota, $jenis)
    {
        if ($jenis=='realisasikan_bo') {
    ...
    public function total_skpd_kab_kota($id_kota)
    {
             $query  = $this->db->query("SELECT id_instansi  FROM master_instansi_kab_kota
                                    WHERE
                                    id_kota = '$id_kota' 
                                        ");
