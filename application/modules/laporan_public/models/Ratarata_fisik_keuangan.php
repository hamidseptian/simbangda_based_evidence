...
(SELECT sum(rp_realisasi_keuangan_bt_bbh + rp_realisasi_keuangan_bt_bbk) from grafik where id_instansi=g.id_instansi and kode_tahap=g.kode_tahap and tahun=g.tahun and bulan <='$bulan') as rp_realisasi_keuangan_bt,";
    ...
    elseif ($realisasi=='fisik_terendah') {
          if ($kategori=='Akumulasi') {
             $order = 'ORDER BY g.realisasi_fisik_akumulasi asc';
          }else{
             $order = 'ORDER BY g.realisasi_fisik_bulanan asc';
    ...
    elseif ($realisasi=='keu_terendah') {
          if ($kategori=='Akumulasi') {
             $order = 'ORDER BY g.realisasi_keuangan_akumulasi asc';
          }else{
             $order = 'ORDER BY g.realisasi_keuangan_bulanan asc';
         
          }
    }
    elseif ($realisasi=='dev_fisik_tertinggi') {
          if ($kategori=='Akumulasi') {
             $order = 'ORDER BY (g.realisasi_fisik_akumulasi - g.target_fisik_akumulasi) asc';
          }else{
             $order = 'ORDER BY (g.realisasi_fisik_bulanan - g.target_fisik_bulanan) asc';
          }
    }

    elseif ($realisasi=='dev_fisik_terendah') {
          if ($kategori=='Akumulasi') {
             $order = 'ORDER BY (g.realisasi_fisik_akumulasi - g.target_fisik_akumulasi) desc';
          }else{
             $order = 'ORDER BY (g.realisasi_fisik_bulanan - g.target_fisik_bulanan) desc';
          }
    }
    elseif ($realisasi=='dev_keu_tertinggi') {
          if ($kategori=='Akumulasi') {
             $order = 'ORDER BY (g.realisasi_keuangan_akumulasi - g.target_keuangan_akumulasi) asc';
          }else{
             $order = 'ORDER BY (g.realisasi_keuangan_bulanan - g.target_keuangan_bulanan) asc';
          }
    }

    elseif ($realisasi=='dev_keu_terendah') {
          if ($kategori=='Akumulasi') {
             $order = 'ORDER BY (g.realisasi_keuangan_akumulasi - g.target_keuangan_akumulasi) desc';
          }else{
             $order = 'ORDER BY (g.realisasi_keuangan_bulanan - g.target_keuangan_bulanan) desc';
          }
    }
...
$query_rp_keu = '';
    ...
    $query  = $this->db->query("SELECT
                                  ...
                                  g.rp_realisasi_keuangan_bulanan, g.last_update
