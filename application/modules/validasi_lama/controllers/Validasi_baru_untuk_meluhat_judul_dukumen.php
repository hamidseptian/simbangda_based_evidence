    ...
    public function dt_paket_swakelola()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            foreach ($list as $lists) {
                ...
                $row[] = $lists->dok;
                $row[] = '<button class="btn ' . $this->validasi_fisik_model->get_status_validasi_paket($lists->id_instansi, $lists->id_paket_pekerjaan, $lists->dok) . ' btn-xs" id="detail-realisasi-fisik" status="collapse" id-instansi="' . $lists->id_instansi . '" id-paket-pekerjaan="' . $lists->id_paket_pekerjaan . '" nama-kpa="' . $this->validasi_fisik_model->get_kpa($lists->id_sub_instansi)['full_name'] . '" nama-pptk="' . $lists->full_name . '" nama-program="' . $this->validasi_fisik_model->get_program_kegiatan($lists->kode_rekening_kegiatan)['nama_program'] . '" nama-kegiatan="' . $this->validasi_fisik_model->get_program_kegiatan($lists->kode_rekening_kegiatan)['nama_kegiatan'] . '" vol="' . $this->vol($lists->id_paket_pekerjaan) . '"><i class="fa fa-plus"></i></button>';
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($dok_realisasi->num_rows() > 0) {
                foreach ($dok_realisasi->result() as $key => $value) {
                    ...
                    $output['data'][$key]['dokumen']            = $value->dokumen;
